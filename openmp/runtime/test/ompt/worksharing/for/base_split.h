...
#pragma omp for schedule(SCHEDULE) nowait
  ...
  // 'ompt_callback_parallel_begin' CHECK-NOT: {{^}}0: Could not register
  // callback 'ompt_callback_parallel_end' CHECK-NOT: {{^}}0: Could not register
  // callback 'ompt_callback_implicit_task' CHECK-NOT: {{^}}0: Could not
  // register callback 'ompt_callback_work'
  ...
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_loop_begin:
  // parallel_id=[[PARALLEL_ID]], parent_task_id={{[0-9]+}},
  // codeptr_ra=[[LOOP_BEGIN_RETURN_ADDRESS:0x[0-f]+]]{{[0-f][0-f]}} CHECK:
  // {{^}}[[MASTER_ID]]: ompt_event_loop_end: parallel_id=[[PARALLEL_ID]],
  // task_id={{[0-9]+}},
  ...
  // parallel_id=[[PARALLEL_ID]], task_id={{[0-9]+}}, invoker={{[0-9]+}},
  // codeptr_ra=[[PARALLEL_RETURN_ADDRESS]] CHECK: {{^}}[[MASTER_ID]]:
  // fuzzy_address={{.*}}[[PARALLEL_RETURN_ADDRESS]]

  // CHECK: {{^}}[[THREAD_ID:[0-9]+]]: ompt_event_loop_begin:
  // parallel_id=[[PARALLEL_ID]], parent_task_id={{[0-9]+}},
  // codeptr_ra=0x{{[0-f]+}} CHECK: {{^}}[[THREAD_ID]]: ompt_event_loop_end:
  ...
  // CHECK-LOOP: {{^}}[[MASTER_ID]]: ompt_event_loop_begin:
  // parallel_id=[[PARALLEL_ID]], parent_task_id={{[0-9]+}},
