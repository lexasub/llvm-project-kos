  ...
  return {
      ...
      {{
       ...
       },
       {
           {"_Z1fv"},
           {"_Z1fSs",
            "_Z1fNSt3__112basic_stringIcNS_11char_traitsIcEENS_"
            "9allocatorIcEEEE",
      ...
      {{
           ...
           // below.)
           {FragmentKind::Name, "NSt3__111char_traitsE",
            "NSt7__cxx1111char_traitsE"},
           // std::__1::allocator == std::allocator
           {FragmentKind::Name, "NSt3__19allocatorE",
      ...
      {{
           ...
           // std::char_traits,
           // due to the std::__cxx11 == std::__1 equivalence, which is what we
           // rely
           // on below.)
       ...
       {
           // f(std::foo)
           {"_Z1fNSt7__cxx113fooE", "_Z1fNSt3__13fooE"},
           // f(std::string)
           {"_Z1fNSt7__cxx1111char_traitsIcEE", "_Z1fNSt3__111char_traitsIcEE",
...
TEST(ItaniumManglingCanonicalizerTest, TestInvalidManglings) {
  ...
  EXPECT_EQ(
      Canonicalizer.addEquivalence(FragmentKind::Name, "N1XcvT_I1AEE", "1f"),
      EquivalenceError::InvalidFirstMangling);
}
...
TEST(ItaniumManglingCanonicalizerTest, TestBadEquivalenceOrder) {
  llvm::ItaniumManglingCanonicalizer Canonicalizer;
  EXPECT_EQ(
      Canonicalizer.addEquivalence(FragmentKind::Type, "N1P1XE", "N1Q1XE"),
  ...
  EXPECT_EQ(
      Canonicalizer.addEquivalence(FragmentKind::Type, "N1C1XE", "N1A1YE"),
      EquivalenceError::Success);
