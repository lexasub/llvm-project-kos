...
void do_assert(bool assert_passed, const char* msg, int line,
               const char* func) {
  if (assert_passed)
    return;
...
struct CreatePointer<Tp*> {
  Tp* operator()() const { return (Tp*)42; }
};
...
void catch_pointer_test() {
  ...
  try {
    ...
    my_assert(catch_ptr == catch2, "Thrown pointer does not match caught ptr");
  } catch (...) {
...
struct generate_tests_imp<Throw, Catch, 0, first> {
  void operator()() { catch_pointer_test<Throw, Catch>(); }
};
