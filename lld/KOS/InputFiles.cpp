...
std::string lld::toString(const kos::InputFile *file) {
  if (!file)
    return "<internal>";
  if (file->parentName.empty() || file->kind() == kos::InputFile::ImportKind)
...
std::vector<MemoryBufferRef> lld::kos::getArchiveMembers(Archive *file) {
...
std::string lld::kos::replaceThinLTOSuffix(StringRef path) {
