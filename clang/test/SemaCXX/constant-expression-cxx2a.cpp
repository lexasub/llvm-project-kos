...
struct PureVirtualCall : Abstract {
  void f();
};                                           // expected-note {{in call to 'Abstract}}
...
struct C : A, C2 {
  A *c = dynamic_cast<A *>(static_cast<C2 *>(this));
};
struct D {
  virtual void d();
};
...
struct Y : X {
  int y;
  virtual constexpr ~Y() { *p++ = 'Y'; }
};
struct Z : Y {
  int z;
  constexpr ~Z() override { *p++ = 'Z'; }
};
union VU {
  constexpr VU() : z() {}
  constexpr ~VU() {}
