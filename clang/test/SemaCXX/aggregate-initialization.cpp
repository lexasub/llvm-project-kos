...
struct S {
  int a;
} s = {(void *)1}; // expected-error {{cannot initialize a member subobject of type 'int' with an rvalue of type 'void *'}}
...
struct S0 {
  int f;
  ~S0() = delete;
}; // expected-note 3 {{'~S0' has been explicitly marked deleted here}}
struct S1 {
  S0 s0;
  int f;
};
...
struct S0 {
  int f;
  ~S0() = delete;
}; // expected-note {{'~S0' has been explicitly marked deleted here}}
