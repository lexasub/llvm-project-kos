...
// parameters.
struct Foo {
  Foo &operator<<(bool) { return *this; }
};
Foo &getFoo();
struct Bar {
  Foo &func() { return getFoo(); }
};
struct Bas {
  void operator&(Foo &) {}
};
...
class PtGuardedBySanityTest {
  ...
  void test1() {
    mu1.Lock();
    if (a == 0)
      doSomething(); // OK, we don't dereference.
    a = 0;
    c = 0;
    if (sa[0] == 42)
      doSomething();
  ...
  void test2() {
    mu1.ReaderLock();
    if (*a == 0)
      doSomething(); // expected-warning {{reading the value pointed to by 'a' requires holding mutex 'mu2'}}
    *a = 0;          // expected-warning {{writing the value pointed to by 'a' requires holding mutex 'mu2' exclusively}}

    if (c->a == 0)
      doSomething(); // expected-warning {{reading the value pointed to by 'c' requires holding mutex 'mu2'}}
  ...
  void test3() {
    mu2.Lock();
    if (*a == 0)
      doSomething(); // expected-warning {{reading variable 'a' requires holding mutex 'mu1'}}
    *a = 0;          // expected-warning {{reading variable 'a' requires holding mutex 'mu1'}}

    if (c->a == 0)
      doSomething(); // expected-warning {{reading variable 'c' requires holding mutex 'mu1'}}
  ...
  void test4() { // Literal arrays
    if (sa[0] == 42)
      doSomething(); // expected-warning {{reading variable 'sa' requires holding mutex 'mu1'}}
    sa[0] = 57;      // expected-warning {{writing variable 'sa' requires holding mutex 'mu1' exclusively}}
    if (sc[0].a == 42)
      doSomething(); // expected-warning {{reading variable 'sc' requires holding mutex 'mu1'}}
...
class SmartPtr_PtGuardedBy_Test {
  ...
  void test2() {
    ...
    if (*sp == 0)
      doSomething(); // expected-warning {{reading variable 'sp' requires holding mutex 'mu1'}}
    *sp = 0;         // expected-warning {{reading variable 'sp' requires holding mutex 'mu1'}}
    ...
    if (sq[0].a == 0)
      doSomething(); // expected-warning {{reading variable 'sq' requires holding mutex 'mu1'}}
  ...
  void test3() {
    ...
    sp.get();
    if (*sp == 0)
      doSomething(); // expected-warning {{reading the value pointed to by 'sp' requires holding mutex 'mu2'}}
    *sp = 0;         // expected-warning {{reading the value pointed to by 'sp' requires holding mutex 'mu2'}}
    ...
    if (sq[0].a == 0)
      doSomething(); // expected-warning {{reading the value pointed to by 'sq' requires holding mutex 'mu2'}}
