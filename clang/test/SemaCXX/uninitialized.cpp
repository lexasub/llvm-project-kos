...
struct Q {
  ...
  int b;
  int &c;
  Q() : a(c = 5), // expected-warning{{reference 'c' is not yet bound to a value when used here}}
...
void if_switch_init_stmt(int k) {
  ...
  if (int n; (n == k || k > 5)) {
  } // expected-warning {{uninitialized}} expected-note {{initialize}}
