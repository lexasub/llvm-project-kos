...
int InitList(int i = {}) {                   // expected-warning {{generalized initializer lists are incompatible with C++98}} \
  ...
  struct {
    int a;
    const int &r;
  } rr = {0, {0}}; // expected-warning {{reference initialized from initializer list is incompatible with C++98}}
...
void RangeFor() {
  ...
  struct Agg {
    int a, b;
  } const &agg = {1, 2}; // expected-warning {{reference initialized from initializer list is incompatible with C++98}}
}
