...
auto fwd_decl_using();
namespace N {
using ::fwd_decl_using;
}
auto fwd_decl_using() { return 0; }
namespace N {
int k = N::fwd_decl_using();
}
...
struct V {
  virtual auto f();
}; // expected-error {{cannot be virtual}}
struct W : V {
  virtual auto f();
};                          // expected-error {{cannot be virtual}}
...
template <typename U> struct A {
  template <typename T = U> friend auto f(A) {
    return [] {};
  }
};
...
void forinit_decltypeauto() {
  for (decltype(auto) forinit_decltypeauto_inner();;) {
  } // expected-warning {{interpreted as a function}} expected-note {{replace}}
}
