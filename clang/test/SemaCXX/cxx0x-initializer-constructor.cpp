...
struct X2 {
  explicit X2(int);
}; // expected-note {{constructor declared here}}
...
struct B {
  B(A);
} b{{0}}; //FIXME: non-conformant. Temporary fix until standard resolution.
...
A a[3] = {
    {1}, {2}}; // expected-error {{call to deleted constructor}} \
...
C c[3] = {
    0, 1}; // expected-error {{ambiguous}} expected-note {{in implicit initialization of array element 2}}
C c2[3] = {
