...
struct Board {
  ...
  constexpr Board(uint64_t State, bool Failed = false) : State(State), Failed(Failed) {}
  ...
  constexpr bool okRecurse(int Row, int Col, int CheckCol) const {
    return Col == CheckCol ? true : getQueenRow(CheckCol) == Row                  ? false
...
constexpr Board buildBoardScan(int N, int Col, int Row, const Board &B) {
  return Row == N ? Board(0, true) : B.ok(Row, Col) ? tryBoard(buildBoardRecurse(N, Col + 1, B.addQueen(Row, Col)), N, Col, Row + 1, B)
