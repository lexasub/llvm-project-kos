...
void test3() {
  ...
  halt() // expected-warning {{will never be executed}}
      ...
      : dead();
  live(),
      float(halt()); // expected-warning {{will never be executed}}
}
...
void test6() {
  ...
  live(),
      S(halt()); // expected-warning {{will never be executed}}
}
...
enum PR19040_test_return_t { PR19040_TEST_FAILURE };
...
__attribute__((noreturn)) void raze();
...
__attribute__((noreturn)) void raze(const A &x);
...
void test_with_paren_silencing(int x) {
  if (false)
    calledFun(); // expected-warning {{will never be executed}} expected-note {{silence by adding parentheses to mark code as explicitly dead}}
  if ((false))
