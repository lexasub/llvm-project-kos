...
constexpr bool j() {
  ...
  d.c.a.x = 3;
  return check(d);
} // expected-note {{assignment to member 'x' of union with active member 'y'}}
...
constexpr int test2() {
  const int k = 0;
  return modify(const_cast<int &>(k));
...
struct ignore {
  template <typename... Ts> constexpr ignore(Ts &&...) {}
};
...
__attribute__((enable_if(indirect_builtin_constant_p("a") == n, "OK"))) int test_in_enable_if() { return 0; }
