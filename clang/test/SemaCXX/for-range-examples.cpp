...
public:
  ...
  value_range_step_iter &operator++() {
    it += step;
    return *this;
  }
};
...
public:
  value_range_step(const T &it, const T &end, const T &step) : it(it), end_(end), step(step) {}
...
void test(A &a) {
  for (int x : a.xs) {
  } // expected-error {{'xs' is a private member of 'test2::A'}}
}
} // namespace test2
...
void f() {
  ...
  for (1 : {1, 2, 3}) {
  } // expected-error {{must declare a variable}}
  for (+x : {1, 2, 3}) {
  } // expected-error {{undeclared identifier}} expected-error {{expected ';'}}
  for (+y : {1, 2, 3}) {
  } // expected-error {{must declare a variable}}
}
} // namespace test4
...
void f() {
  int arr[5], b;
  for (a : arr) {
  } // expected-error {{requires type for loop variable}}
  ...
  for (d alignas(1) : arr) {
  } // expected-error {{requested alignment is less than minimum}} expected-error {{requires type for loop variable}}
  for (e [[deprecated]] : arr) {
    e = 0;
  } // expected-warning {{deprecated}} expected-note {{here}} expected-error {{requires type for loop variable}}
}
} // namespace test7
