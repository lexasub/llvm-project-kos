...
void f1() {
  ...
  void (^x)() = ^{
    void A::Af();
  }; // expected-error {{definition or redeclaration of 'Af' not allowed inside a block}}
}
...
void f3() {
  ...
  }
  {
    typedef int A;
    A::ax = 0;
  } // expected-error{{'A' (aka 'int') is not a class, namespace, or enumeration}}
  {
    typedef A::C A;
    A::ax = 0;
  } // expected-error {{no member named 'ax'}}
  {
    typedef A::C A;
    A::cx = 0;
  }
...
struct S2c {
  C1::C2 f(C1
           : C2); // expected-error{{unexpected ':' in nested name specifier; did you mean '::'?}}
};
...
int (*X2)() = (B1
               : B2 ? F<1> : F<2>); // expected-error{{unexpected ':' in nested name specifier; did you mean '::'?}}
