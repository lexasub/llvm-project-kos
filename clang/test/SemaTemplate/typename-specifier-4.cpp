...
#endif
           make_pair::apply<int, float>>::value
           ? 1
           : -1];
...
struct B {
  typedef int type;
}; // expected-note 2{{member type 'int' found by ambiguous name lookup}}
struct C {
  typedef const int type;
}; // expected-note 2{{member type 'const int' found by ambiguous name lookup}}
