...
struct A {
  int n;
  constexpr A(int n = 0) : n(n) {}
  constexpr A(const A &a) : n(a.n + 1) {}
};
...
template <A V> requires Same<V, A{V.arr[0], V.arr[0]}> struct X<V> {
