...
template <typename T>
struct S2 {
  template <typename = void> requires(value, true) static constexpr auto f() requires(value, true) {
  }
};
...
template <typename T>
struct S3 {
  template <typename... Args> requires true static constexpr void f(Args...) {}
};
