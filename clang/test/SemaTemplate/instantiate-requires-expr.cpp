...
template <typename T> requires false_v < requires(T t) { requires is_same_v<decltype(t), int>; }
>
...
template <typename... Ts> requires false_v < requires(Ts... ts) { requires((sizeof(ts) == 2) && ...); }
>
...
template <typename... Ts> requires false_v<(requires(Ts ts) { requires sizeof(ts) != 0; } && ...)>
...
template <typename T> requires false_v < requires { typename T::type; }
...
struct contains_template {
  template <typename U> requires is_same_v<contains_template<T>, U> using temp = int;
};

template <typename T> requires false_v < requires { typename T::template temp<T>; }
>
...
template <typename... Ts> requires false_v<(requires { typename Ts::type; } && ...)>
...
template <typename... Ts> requires false_v<(requires { typename void_t<Ts>; } && ...)> // expected-note{{because 'false_v<requires { typename void_t<int>; } && requires { typename void_t<short>; }>' evaluated to false}}
    struct r6 {};

using r6i = r6<int, short>; // expected-error{{constraints not satisfied for class template 'r6' [with Ts = <int, short>]}}

template <typename... Ts> requires false_v<(requires { typename Ts::template aaa<Ts>; } && ...)>
...
template <typename T> requires false_v < requires {
  sizeof(T);
  {sizeof(T)};
}
>
...
struct a {
  template <typename U> requires(
      requires { sizeof(T::a); }, false) // expected-note{{because 'requires { <<error-expression>>; } , false' evaluated to false}}
      struct r {};
};
...
template <typename T> requires false_v < requires(T t) {
  { t }
  ->C1<T>;
}
>
...
struct b {
  template <typename U> requires(
      requires { { 0 } -> C1<typename T::a>; }, false) // expected-note{{because 'requires { { 0 } -> <<error-type>>; } , false' evaluated to false}}
      struct r {};
};

using bri = b<int>::r<short>; // expected-error{{constraints not satisfied for class template 'r' [with U = short]}}

template <typename... Ts> requires false_v<(requires { { 0 } noexcept -> C1<Ts>; } && ...)>
...
template <typename T> requires false_v < requires { requires sizeof(T) == 2; }
> // expected-note{{because 'false_v<requires { requires sizeof(int) == 2; }>' evaluated to false}}
    struct r1 {};
...
// Parameter pack inside expr
template <typename... Ts> requires false_v<(requires { requires sizeof(Ts) == 0; } && ...)>
...
template <typename... Ts> requires false_v<(requires { requires sizeof(Ts) == 0; sizeof(Ts); } && ...)>
...
template <typename... Ts> requires false_v<(requires(Ts t) { requires sizeof(t) == 0; t++; } && ...)>
...
template <typename T> requires false_v<(requires(T t) { T{t}; })> // T{t} creates an "UnevaluatedList" context.
