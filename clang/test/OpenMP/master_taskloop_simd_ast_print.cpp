...
#pragma omp taskgroup task_reduction(+ \
                                     : d) allocate(d)
#pragma omp master taskloop simd allocate(d) if (taskloop                                                                                                                    \
                                                 : argc > N) default(shared) untied priority(N) safelen(N) linear(c) aligned(ptr) grainsize(N) reduction(+                   \
                                                                                                                                                         : g) in_reduction(+ \
                                                                                                                                                                           : d)
...
#pragma omp taskgroup task_reduction(+ \
                                     : d)
#pragma omp master taskloop simd if (taskloop                                                                \
                                     : a) default(none) shared(a) final(b) priority(5) safelen(8) linear(b), \
    aligned(argv) num_tasks(argc) reduction(*                                                                \
                                            : g) in_reduction(+                                              \
                                                              : d)
...
#pragma omp parallel
#ifdef OMP5
#pragma omp master taskloop simd private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if (simd                                                                       \
                                                                                                                      : argc) mergeable priority(argc) simdlen(16) grainsize(argc) reduction(max \
                                                                                                                                                                                             : a, e) nontemporal(argc, c, d) order(concurrent)
#else
#pragma omp master taskloop simd private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if (argc) mergeable priority(argc) simdlen(16) grainsize(argc) reduction(max \
                                                                                                                                                                                           : a, e)
#endif // OMP5
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j)
