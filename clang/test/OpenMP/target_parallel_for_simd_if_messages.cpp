...
#pragma omp target parallel for simd if (target               \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd if (parallel \
                                         : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp target parallel for simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd if (parallel         \
                                         : argc) if (simd \
                                                     : argc) // omp45-error {{directive name modifier 'simd' is not allowed for '#pragma omp target parallel for simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd if (target             \
                                         : argc) if (target \
                                                     : argc) // expected-error {{directive '#pragma omp target parallel for simd' cannot contain more than one 'if' clause with 'target' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd if (parallel             \
                                         : argc) if (parallel \
                                                     : argc) // expected-error {{directive '#pragma omp target parallel for simd' cannot contain more than one 'if' clause with 'parallel' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd if (target \
                                         : argc) if (argc) // omp45-error {{expected 'parallel' directive name modifier}} omp50-error {{expected one of 'parallel' or 'simd' directive name modifiers}} expected-note {{previous clause with directive name modifier specified here}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd if (target               \
                                         : argc) if (parallel \
                                                     : argc) if (argc) // omp45-error {{no more 'if' clause is allowed}} omp50-error {{expected 'simd' directive name modifier}} expected-note {{previous clause with directive name modifier specified here}} expected-note {{previous clause with directive name modifier specified here}}
  for (i = 0; i < argc; ++i)
    foo();
