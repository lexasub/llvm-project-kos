...
#pragma omp target teams distribute simd aligned(B \
                                                 : bfoo())
  for (int i = 0; i < 10; ++i)
    ;

#pragma omp target teams distribute simd aligned(B::ib:B \
...
#pragma omp target teams distribute simd aligned(z:B \
                                                 : bfoo()) // expected-error {{unexpected ':' in nested name specifier; did you mean '::'?}}
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp target teams distribute simd aligned(X::x \
...
#pragma omp target teams distribute simd aligned(B, rp, ::z \
...
#pragma omp target teams distribute simd aligned(arr \
...
template <int LEN> int test_warn() {
  int *ind2 = 0;
#pragma omp target teams distribute simd aligned(ind2 \
