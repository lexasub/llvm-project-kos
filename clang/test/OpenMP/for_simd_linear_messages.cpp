...
#pragma omp for simd linear(i, lin \
...
#pragma omp for simd linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'?}}
#pragma omp for simd linear(z:B \
                            : ib)
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp for simd linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp for simd linear(X::x \
                            : ::z)
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp for simd linear(ind2 \
                            : L)
  for (i = 0; i < num; ++i) {
    T cur = arr[(int)ind2];
    ind2 += L;
...
#pragma omp for simd linear(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp for simd linear(argc : 5) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp parallel
  {
    ...
    long z;
    int i;
#pragma omp for simd linear(z, v \
                            : i)
    for (int k = 0; k < argc; ++k) {
      i = k;
      v += i;
