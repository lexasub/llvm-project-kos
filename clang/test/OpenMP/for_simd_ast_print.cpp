...
// CHECK: T lin = 0;
#ifdef OMP5
#pragma omp for simd private(val) safelen(7) linear(lin \
                                                    : -5) lastprivate(res) simdlen(5) allocate(res) if (res) nontemporal(res, val, lin) order(concurrent)
#else
#pragma omp for simd private(val) safelen(7) linear(lin \
                                                    : -5) lastprivate(res) simdlen(5) allocate(res)
#endif
...
template <int LEN> struct S2 {
  static void func(int n, float *a, float *b, float *c) {
    int k1 = 0, k2 = 0;
#pragma omp for simd allocate(k1) safelen(LEN) linear(k1, k2           \
                                                      : LEN) aligned(a \
                                                                     : LEN) simdlen(LEN)
    for (int i = 0; i < n; i++) {
      c[i] = a[i] + b[i];
...
#ifdef OMP5
#pragma omp for simd aligned(a                                                                       \
                             : CLEN) linear(a                                                        \
                                            : CLEN) safelen(CLEN) collapse(1) simdlen(CLEN) if (simd \
                                                                                                : a)
#else
#pragma omp for simd aligned(a                \
                             : CLEN) linear(a \
