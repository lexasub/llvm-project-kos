...
// CHECK-NEXT: a = 2;
#pragma omp target parallel for simd allocate(d) private(argc, b), firstprivate(c, d), lastprivate(d, f) collapse(N) schedule(static, N) ordered if (parallel                                                     \
...
#pragma omp target parallel for simd default(none), private(argc, b) firstprivate(argv) shared(d) if (parallel                                                                              \
                                                                                                      : argc > 0) num_threads(N) proc_bind(master) reduction(+                              \
...
#pragma omp target parallel for simd if (N) num_threads(s) proc_bind(close) reduction(^                                      \
                                                                                      : e, f, arr [0:N][:argc]) reduction(&& \
                                                                                                                          : h)
  for (T i = 0; i < 2; ++i) {
  }
...
#pragma omp target parallel for simd if (target                                                                                           \
                                         : argc > 0) if (simd                                                                             \
                                                         : argc) nontemporal(argc, c, d) order(concurrent) allocate(omp_high_bw_mem_alloc \
                                                                                                                    : f) private(f) uses_allocators(omp_high_bw_mem_alloc)
#else
#pragma omp target parallel for simd if (target \
                                         : argc > 0)
#endif // OMP5
...
#pragma omp target parallel for simd if (parallel \
                                         : argc > 0)
  for (T i = 0; i < 2; ++i) {
  }
...
#pragma omp target parallel for simd map(always, alloc \
                                         : i)
  for (T i = 0; i < 2; ++i) {
  }
...
#pragma omp target parallel for simd defaultmap(tofrom \
                                                : scalar)
  for (T i = 0; i < 2; ++i) {
  }
...
#pragma omp target parallel for simd aligned(arr1 \
                                             : N - 1)
  for (T i = 0; i < N; ++i) {
  }
...
#pragma omp target parallel for simd private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) schedule(auto) ordered if (target                                                                   \
                                                                                                                                       : argc) num_threads(a) default(shared) shared(e) reduction(+             \
                                                                                                                                                                                                  : h) linear(a \
                                                                                                                                                                                                              : -5)
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j)
...
#pragma omp target parallel for simd default(none), private(argc, b) firstprivate(argv) shared(d) if (parallel                                                                              \
                                                                                                      : argc > 0) num_threads(5) proc_bind(master) reduction(+                              \
                                                                                                                                                             : c, arr1[argc]) reduction(max \
                                                                                                                                                                                        : e, arr[:5] [0:10])
  for (int i = 0; i < 2; ++i) {
  }
...
#pragma omp target parallel for simd if (5) num_threads(s) proc_bind(close) reduction(^                                      \
                                                                                      : e, f, arr [0:5][:argc]) reduction(&& \
                                                                                                                          : h)
  for (int i = 0; i < 2; ++i) {
  }
