...
#pragma omp target teams distribute parallel for simd device(global + a) depend(in                   \
                                                                                : global) depend(out \
                                                                                                 : a, b, cn[4])
  for (int i = 0; i < 10; ++i) {
  }
...
#pragma omp target teams distribute parallel for simd device(global + a) nowait depend(inout                       \
...
// CHECK:       call void @__kmpc_omp_task_complete_if0(%struct.ident_t* [[ID]], i32 [[GTID]], i8* [[TASK]])
#pragma omp target teams distribute parallel for simd if (0) firstprivate(global) depend(out \
                                                                                         : global)
