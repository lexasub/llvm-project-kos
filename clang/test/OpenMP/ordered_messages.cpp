...
#pragma omp ordered depend(sink \
                           : i) // expected-error {{'ordered' directive with 'depend' clause cannot be closely nested inside ordered region without specified parameter}}
  }
...
#pragma omp ordered depend(sink \
                           : // expected-error {{expected ')'}} expected-note {{to match this '('}} expected-error {{expected expression}} expected-error {{expected 'i' loop iteration variable}}
#pragma omp ordered depend(sink \
                           : i // expected-error {{expected ')'}} expected-note {{to match this '('}} expected-error {{expected 'j' loop iteration variable}}
#pragma omp ordered depend(sink \
                           : i) // expected-error {{expected 'j' loop iteration variable}}
...
#pragma omp ordered depend(source) // expected-error {{'#pragma omp ordered' with 'depend' clause cannot be an immediate substatement}}
      ...
      if (i == j)
#pragma omp ordered depend(sink \
                           : i, j) // expected-error {{'#pragma omp ordered' with 'depend' clause cannot be an immediate substatement}}
        ;
...
#pragma omp ordered depend(in \
                           : i) // expected-error {{expected 'source' or 'sink' in OpenMP clause 'depend'}} expected-error {{'ordered' directive without any clauses cannot be closely nested inside ordered region with specified parameter}}
#pragma omp ordered depend(sink \
                           : i, j)
#pragma omp ordered depend(sink \
                           : j, i) // expected-error {{expected 'i' loop iteration variable}} expected-error {{expected 'j' loop iteration variable}}
#pragma omp ordered depend(sink \
                           : i, j, k) // expected-error {{unexpected expression: number of expressions is larger than the number of associated loops}}
#pragma omp ordered depend(sink \
                           : i + foo(), j / 4) // expected-error {{integral constant expression}} expected-error {{expected '+' or '-' operation}}
#if __cplusplus >= 201103L
// expected-note@-2 {{non-constexpr function 'foo' cannot be used in a constant expression}}
#endif
#pragma omp ordered depend(sink \
                           : i * 0, j - 4) // expected-error {{expected '+' or '-' operation}}
#pragma omp ordered depend(sink                                \
                           : i - 0, j + sizeof(T)) depend(sink \
                                                          : i - 0, j + sizeof(T))
#pragma omp ordered depend(sink \
                           : i - 0, j + sizeof(T)) depend(source) // expected-error {{'depend(source)' clause cannot be mixed with 'depend(sink:vec)' clauses}}
#pragma omp ordered depend(source) depend(sink \
                                          : i - 0, j + sizeof(T)) // expected-error {{'depend(sink:vec)' clauses cannot be mixed with 'depend(source)' clause}}
    }
  }
#pragma omp ordered depend(source) // expected-error {{'ordered' directive with 'depend' clause cannot be closely nested inside ordered region without specified parameter}}
#pragma omp ordered depend(sink \
                           : k) // expected-error {{'ordered' directive with 'depend' clause cannot be closely nested inside ordered region without specified parameter}}
  return T();
}
...
#pragma omp ordered depend(in \
                           : i) // expected-error {{expected 'source' or 'sink' in OpenMP clause 'depend'}} expected-error {{'ordered' directive without any clauses cannot be closely nested inside ordered region with specified parameter}}
#pragma omp ordered depend(sink \
                           : i, j) allocate(i) // expected-error {{unexpected OpenMP clause 'allocate' in directive '#pragma omp ordered'}}
#pragma omp ordered depend(sink \
                           : j, i) // expected-error {{expected 'i' loop iteration variable}} expected-error {{expected 'j' loop iteration variable}}
#pragma omp ordered depend(sink \
                           : i, j, k) // expected-error {{unexpected expression: number of expressions is larger than the number of associated loops}}
...
#pragma omp ordered depend(sink \
                           : i * 0, j - 4) // expected-error {{expected '+' or '-' operation}}
#pragma omp ordered depend(sink                                  \
                           : i - 0, j + sizeof(int)) depend(sink \
                                                            : i - 0, j + sizeof(int))
#pragma omp ordered depend(sink \
                           : i - 0, j + sizeof(int)) depend(source) // expected-error {{'depend(source)' clause cannot be mixed with 'depend(sink:vec)' clauses}}
#pragma omp ordered depend(source) depend(sink \
                                          : i - 0, j + sizeof(int)) // expected-error {{'depend(sink:vec)' clauses cannot be mixed with 'depend(source)' clause}}
    }
  }
...
#pragma omp ordered depend(sink \
    ...
    int j;
#pragma omp ordered depend(sink \
                           : i, j) // expected-error {{expected loop iteration variable}}
    foo();
  }
