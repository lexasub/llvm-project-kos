...
#pragma omp target teams distribute parallel for private(a) private(this->a) private(T::a) allocate(omp_cgroup_mem_alloc \
                                                                                                    : a) uses_allocators(omp_cgroup_mem_alloc)
    for (int k = 0; k < a.a; ++k) {
      ++this->a.a;
...
#pragma omp target teams distribute parallel for private(a) private(this->a)
  ...
  void foo() {
    int b, argv, d, c, e, f;
#pragma omp target teams distribute parallel for default(none), private(b) firstprivate(argv) shared(d) reduction(+                  \
                                                                                                                  : c) reduction(max \
...
#pragma omp target teams distribute parallel for private(a) private(this->a)
  ...
  void bar() {
    int b, argv, d, c, e, f8;
#pragma omp target teams distribute parallel for allocate(b) default(none), private(b) firstprivate(argv) shared(d) reduction(+                  \
                                                                                                                              : c) reduction(max \
                                                                                                                                             : e) num_teams(f8) thread_limit(d) allocate(e)
    for (int k = 0; k < a.a; ++k)
      ++a.a;
  }
};
...
// CHECK-NEXT: for (int i = 0; i < 10; ++i)
// CHECK-NEXT: foo();
#pragma omp target teams distribute parallel for default(none), private(b) firstprivate(argc) shared(d) reduction(+                  \
                                                                                                                  : c) reduction(max \
                                                                                                                                 : e) num_teams(f) thread_limit(d)
  for (int k = 0; k < 10; ++k)
    e += d + argc;
