...
#pragma omp target if (target \
  ...
  foo();
#pragma omp target if (target \
                       : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp target'}}
  foo();
#pragma omp target if (target             \
                       : argc) if (target \
                                   : argc) // expected-error {{directive '#pragma omp target' cannot contain more than one 'if' clause with 'target' name modifier}}
  foo();
#pragma omp target if (target \
