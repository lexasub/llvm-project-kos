...
#pragma omp target teams distribute parallel for simd lastprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp target teams distribute parallel for simd lastprivate(conditional                     \
...
#pragma omp target teams distribute parallel for simd lastprivate(h) // expected-error {{threadprivate or thread local variable cannot be lastprivate}}
  ...
  int v = 0;
#pragma omp target teams distribute parallel for simd lastprivate(i) allocate(omp_thread_mem_alloc \
                                                                              : i) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'target teams distribute parallel for simd' directive}} le50-error {{allocator must be specified in the 'uses_allocators' clause}}
  for (int k = 0; k < argc; ++k) {
    i = k;
    v += i;
  }
