...
#pragma omp target parallel for map(to:) // expected-error {{expected expression}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for map(from \
...
#pragma omp target parallel for map(tofrom \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for map(T \
...
#pragma omp target parallel for map(always, tofrom \
                                    : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for map(always \
...
#pragma omp target parallel for map(tofrom, always \
                                    : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for map(always, tofrom \
