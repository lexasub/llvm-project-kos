...
#pragma omp target defaultmap(tofrom                      \
                              : scalar) defaultmap(tofrom \
                                                   : scalar) // omp45-error {{directive '#pragma omp target' cannot contain more than one 'defaultmap' clause}} omp5-error {{at most one defaultmap clause for each variable-category can appear on the directive}} omp51-error {{at most one defaultmap clause for each variable-category can appear on the directive}}

  foo();

#if OMP5
#pragma omp target defaultmap(tofrom \
                              : something) // omp5-error {{expected 'scalar', 'aggregate', 'pointer' in OpenMP clause 'defaultmap'}}}
  foo();
#pragma omp target defaultmap(tofrom                  \
                              : scalar) defaultmap(to \
                                                   : scalar) // omp5-error {{at most one defaultmap clause for each variable-category can appear on the directive}}
  foo();
#pragma omp target defaultmap(alloc                                                   \
                              : pointer) defaultmap(to                                \
                                                    : scalar) defaultmap(firstprivate \
                                                                         : pointer) // expected-error {{at most one defaultmap clause for each variable-category can appear on the directive}}
  foo();
#pragma omp target defaultmap(none \
                              : pointer) // expected-note {{explicit data sharing attribute, data mapping attribute, or is_device_ptr clause requested here}}
...
#pragma omp target defaultmap(none                       \
                              : pointer) defaultmap(none \
                                                    : scalar) map(argc) // expected-note {{explicit data sharing attribute, data mapping attribute, or is_device_ptr clause requested here}}
...
#pragma omp target defaultmap(none                       \
                              : pointer) defaultmap(none \
                                                    : scalar) map(g) // expected-note {{explicit data sharing attribute, data mapping attribute, or is_device_ptr clause requested here}}
...
#pragma omp target defaultmap(none                                            \
                              : pointer) defaultmap(none                      \
                                                    : scalar) defaultmap(none \
                                                                         : aggregate) map(g) firstprivate(argc) // expected-note {{explicit data sharing attribute, data mapping attribute, or is_device_ptr clause requested here}}
...
#pragma omp target defaultmap(none \
                              : aggregate) // expected-note {{explicit data sharing attribute, data mapping attribute, or is_device_ptr clause requested here}}
#pragma omp parallel
  baz.bar.a += argc; // expected-error {{variable 'baz' must have explicitly specified data sharing attributes, data mapping attributes, or in an is_device_ptr clause}}
#endif
...
#pragma omp target defaultmap(present \
                              : somthing) // omp51-error {{expected 'scalar', 'aggregate', 'pointer' in OpenMP clause 'defaultmap'}}}
  foo();
#pragma omp target defaultmap(present                      \
                              : scalar) defaultmap(present \
                                                   : scalar) // omp51-error {{at most one defaultmap clause for each variable-category can appear on the directive}}
  foo();
#endif
