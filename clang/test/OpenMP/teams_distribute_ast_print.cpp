...
#pragma omp teams distribute private(a) private(this->a)
  ...
  void foo() {
    int b, argv, d, c, e, f;
#pragma omp target
#pragma omp teams distribute default(none), private(b) firstprivate(argv) shared(d) reduction(+                  \
                                                                                              : c) reduction(max \
...
#pragma omp teams distribute private(a) private(this->a)
  ...
  void bar() {
    int b, argv, d, c, e, f;
#pragma omp target
#pragma omp teams distribute allocate(omp_thread_mem_alloc                                                                            \
                                      : argv) default(none),                                                                          \
    private(b) firstprivate(argv) shared(d) reduction(+                                                                               \
                                                      : c) reduction(max                                                              \
                                                                     : e) num_teams(f) thread_limit(d) allocate(omp_default_mem_alloc \
                                                                                                                : c) shared(omp_default_mem_alloc, omp_thread_mem_alloc)
    for (int k = 0; k < a.a; ++k)
      ++a.a;
  }
};
...
// CHECK-NEXT: foo();
#pragma omp target
#pragma omp teams distribute default(none), private(b) firstprivate(argc) shared(d) reduction(+                  \
                                                                                              : c) reduction(max \
                                                                                                             : e) num_teams(f) thread_limit(d)
  for (int k = 0; k < 10; ++k)
    e += d + argc;
