...
template <class T> struct S {
  ...
  T result(T *v) const {
    ...
    // CHECK: T lin = 0;
    // CHECK: T &ref = res;
#pragma omp simd allocate(res) private(val) safelen(7) linear(lin \
                                                              : -5) lastprivate(res) simdlen(5) linear(ref(ref))
...
template <int LEN> struct S2 {
  static void func(int n, float *a, float *b, float *c) {
    int k1 = 0, k2 = 0;
#pragma omp simd safelen(LEN) linear(k1, k2           \
                                     : LEN) aligned(a \
                                                    : LEN) simdlen(LEN) allocate(k1)
    for (int i = 0; i < n; i++) {
      c[i] = a[i] + b[i];
      c[k1] = a[k1] + b[k1];
      c[k2] = a[k2] + b[k2];
...
// CHECK-NEXT: *a = 2;
#ifdef OMP5
#pragma omp simd private(argc, b), lastprivate(d, f) collapse(2) aligned(a : 4) if (simd                                                 \
                                                                                    : a) nontemporal(argc, c, d) lastprivate(conditional \
                                                                                                                             : e, f) order(concurrent)
...
#pragma omp simd aligned(a                                                                              \
                         : CLEN) linear(a                                                               \
                                        : CLEN) safelen(CLEN) collapse(1) simdlen(CLEN) linear(val(ref) \
                                                                                               : CLEN) if (a)
