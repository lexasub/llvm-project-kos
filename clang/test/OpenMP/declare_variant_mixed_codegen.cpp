...
#pragma omp declare variant(prio1) match(implementation = {vendor(score(2)  \
                                                                  : llvm)}, \
                                         device = {kind(cpu, host)})
#pragma omp declare variant(prio) match(implementation = {vendor(score(1)  \
                                                                 : llvm)}, \
                                        device = {kind(cpu)})
int prio_() { return 20; }
...
#pragma omp declare variant(prio4) match(implementation = {vendor(score(5) \
                                                                  : llvm)})
#pragma omp declare variant(prio2) match(implementation = {vendor(score(8)  \
                                                                  : llvm)}, \
                                         device = {kind(cpu, host)})
#pragma omp declare variant(prio3) match(implementation = {vendor(score(7)  \
                                                                  : llvm)}, \
                                         device = {kind(cpu)})
