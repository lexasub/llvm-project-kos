...
#pragma omp target
#pragma omp teams
#pragma omp distribute firstprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp teams reduction(+ \
                            : argc) // expected-note {{defined as reduction}}
#pragma omp distribute firstprivate(argc) // expected-error {{firstprivate variable must be shared}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target
