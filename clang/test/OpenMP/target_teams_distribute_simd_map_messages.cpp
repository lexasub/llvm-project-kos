...
#pragma omp target teams distribute simd map(to:) // expected-error {{expected expression}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute simd map(from \
...
#pragma omp target teams distribute simd map(tofrom \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute simd map(T \
...
#pragma omp target teams distribute simd map(always, tofrom \
                                             : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute simd map(always \
...
#pragma omp target teams distribute simd map(tofrom, always \
                                             : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute simd map(always, tofrom \
