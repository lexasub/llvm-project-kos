...
#pragma omp target teams distribute parallel for simd if (parallel \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (target \
                                                          : // expected-error {{expected expression}} expected-error {{expected ')'}} expected-note {{to match this '('}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (parallel \
...
#pragma omp target teams distribute parallel for simd if (parallel         \
                                                          : argc) if (simd \
                                                                      : argc) // omp45-error {{directive name modifier 'simd' is not allowed for '#pragma omp target teams distribute parallel for simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (parallel             \
                                                          : argc) if (parallel \
                                                                      : argc) // expected-error {{directive '#pragma omp target teams distribute parallel for simd' cannot contain more than one 'if' clause with 'parallel' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (target             \
                                                          : argc) if (target \
                                                                      : argc) // expected-error {{directive '#pragma omp target teams distribute parallel for simd' cannot contain more than one 'if' clause with 'target' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (parallel \
                                                          : argc) if (argc) // expected-note {{previous clause with directive name modifier specified here}} omp45-error {{expected 'target' directive name modifier}} omp50-error {{expected one of 'target' or 'simd' directive name modifiers}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (target \
                                                          : argc) if (argc) // expected-note {{previous clause with directive name modifier specified here}} omp45-error {{expected 'parallel' directive name modifier}} omp50-error {{expected one of 'parallel' or 'simd' directive name modifiers}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (distribute \
                                                          : argc) // expected-error {{directive name modifier 'distribute' is not allowed for '#pragma omp target teams distribute parallel for simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute parallel for simd if (target               \
                                                          : argc) if (parallel \
