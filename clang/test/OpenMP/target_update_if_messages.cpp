...
#pragma omp target update to(n) if (target update \
                                    : // expected-error {{expected expression}} expected-error {{expected ')'}} expected-note {{to match this '('}}
#pragma omp target update from(n) if (target update \
                                      : argc // expected-error {{expected ')'}} expected-note {{to match this '('}}
#pragma omp target update to(n) if (target update \
                                    : argc + n)
#pragma omp target update from(n) if (target update \
                                      : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp target update'}}
#pragma omp target update to(n) if (target update             \
                                    : argc) if (target update \
                                                : argc) // expected-error {{directive '#pragma omp target update' cannot contain more than one 'if' clause with 'target update' name modifier}}
#pragma omp target update from(n) if (target update \
...
#pragma omp target update from(m) if (target update \
                                      : // expected-error {{expected expression}} expected-error {{expected ')'}} expected-note {{to match this '('}}
#pragma omp target update to(m) if (target update \
                                    : argc // expected-error {{expected ')'}} expected-note {{to match this '('}}
#pragma omp target update from(m) if (target update \
                                      : argc + m)
#pragma omp target update to(m) if (target update \
