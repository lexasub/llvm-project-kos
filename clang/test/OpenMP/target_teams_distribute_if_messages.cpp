...
#pragma omp target teams distribute if (target \
...
#pragma omp target teams distribute if (teams \
                                        : argc) // expected-error {{directive name modifier 'teams' is not allowed for '#pragma omp target teams distribute'}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (distribute \
                                        : argc) // expected-error {{directive name modifier 'distribute' is not allowed for '#pragma omp target teams distribute'}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (argc) if (teams \
                                                  : argc) // expected-note {{previous clause with directive name modifier specified here}} expected-error {{directive name modifier 'teams' is not allowed for '#pragma omp target teams distribute'}} expected-error {{no more 'if' clause is allowed}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (argc) if (distribute \
                                                  : argc) // expected-note {{previous clause with directive name modifier specified here}} expected-error {{directive name modifier 'distribute' is not allowed for '#pragma omp target teams distribute'}} expected-error {{no more 'if' clause is allowed}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (distribute \
                                        : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp target teams distribute'}} expected-error {{directive name modifier 'distribute' is not allowed for '#pragma omp target teams distribute'}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (target             \
                                        : argc) if (target \
                                                    : argc) // expected-error {{directive '#pragma omp target teams distribute' cannot contain more than one 'if' clause with 'target' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (target \
                                        : argc) if (argc) // expected-note {{previous clause with directive name modifier specified here}} expected-error {{no more 'if' clause is allowed}}
  for (i = 0; i < argc; ++i)
    foo();

#pragma omp target teams distribute if (target                 \
                                        : argc) if (distribute \
                                                    : argc) if (argc) // expected-note 2 {{previous clause with directive name modifier specified here}} expected-error {{directive name modifier 'distribute' is not allowed for '#pragma omp target teams distribute'}} expected-error {{expected one of  directive name modifiers}}
  for (i = 0; i < argc; ++i)
    foo();
