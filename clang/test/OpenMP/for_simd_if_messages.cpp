...
#pragma omp for simd if (simd \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp for simd if (target \
                         : argc) // expected-error {{directive name modifier 'target' is not allowed for '#pragma omp for simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp for simd if (simd             \
                         : argc) if (simd \
                                     : argc) // expected-error {{directive '#pragma omp for simd' cannot contain more than one 'if' clause with 'simd' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp for simd if (simd \
...
#pragma omp for simd if (simd \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp for simd if (simd \
                         : argc + z) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp for simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp for simd if (simd             \
                         : argc) if (simd \
