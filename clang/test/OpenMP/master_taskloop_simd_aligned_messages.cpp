...
void test_aligned_colons(int *&rp) {
  int *B = 0;
#pragma omp master taskloop simd aligned(B \
                                         : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'}}
#pragma omp master taskloop simd aligned(B::ib:B \
                                         : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp master taskloop simd aligned(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'?}}
#pragma omp master taskloop simd aligned(z:B \
                                         : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp master taskloop simd aligned(B \
...
#pragma omp master taskloop simd aligned(arr \
                                         : L)
  for (i = 0; i < num; ++i) {
    T cur = arr[(int)ind2];
    ind2 += L;
...
template <int LEN> int test_warn() {
  int *ind2 = 0;
// expected-error@+1 {{argument to 'aligned' clause must be a strictly positive integer value}}
#pragma omp master taskloop simd aligned(ind2 \
                                         : LEN)
  for (int i = 0; i < 100; i++) {
    ind2 += LEN;
  }
