...
#pragma omp target parallel private(argc argv) // expected-error {{expected ',' or ')' in 'private' clause}}
  {}
#pragma omp target parallel private(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp target parallel private(argv[1]) // expected-error {{expected variable name}}
  {}
#pragma omp target parallel allocate(omp_thread_mem_alloc \
                                     : ba) private(ba) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'target parallel' directive}} expected-error {{allocator must be specified in the 'uses_allocators' clause}}
  {}
