...
#pragma omp target exit data if (1 + 3 - 5) device(arg) map(from \
                                                            : gc) nowait
  { ++arg; }
...
// CK1-32-DAG: [[CSVAL032]] = mul nuw i32 %{{[^,]+}}, 4
// CK1: %{{.+}} = add nsw i32 %{{[^,]+}}, 1
#pragma omp target exit data map(close, from \
                                 : lb)
  { ++arg; }
...
// CK1-32-DAG: [[CSVAL032]] = mul nuw i32 %{{[^,]+}}, 4
// CK1: %{{.+}} = add nsw i32 %{{[^,]+}}, 1
#pragma omp target exit data map(always close, from \
                                 : lb)
  { ++arg; }
}
...
struct STT {
  ...
  T foo(T arg) {
// Region 00
#pragma omp target exit data map(always close, release \
                                 : b [1:3]) if (a > 123) device(arg)
    { arg++; }
    return arg;
  }
