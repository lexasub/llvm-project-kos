...
#pragma omp simd if (simd \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp simd if (target \
                     : argc) // expected-error {{directive name modifier 'target' is not allowed for '#pragma omp simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp simd if (simd             \
                     : argc) if (simd \
                                 : argc) // expected-error {{directive '#pragma omp simd' cannot contain more than one 'if' clause with 'simd' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp simd if (simd \
