...
#pragma omp taskgroup allocate(b) task_reduction(+ \
                                                 : b)
#pragma omp task private(a) private(this->a) private(T::a) in_reduction(+                                                                                                                           \
                                                                        : this->b) allocate(b) depend(depobj                                                                                        \
                                                                                                      : x) detach(evt) depend(iterator(i = 0 : 10 : 1, T *k = &a                                    \
                                                                                                                                       : &b),                                                       \
                                                                                                                              in                                                                    \
                                                                                                                              : c[i], d[(int)(k - &a)]) affinity(iterator(i = 0 : 10 : 1, T *k = &a \
                                                                                                                                                                          : &b)                     \
                                                                                                                                                                 : c[i], d[(int)(k - &a)])
    for (int k = 0; k < a.a; ++k)
      ++this->a.a;
...
#pragma omp taskgroup task_reduction(- \
                                     : argc)
#pragma omp task if (C) mergeable priority(C) in_reduction(- \
                                                           : argc)
  foo();
  return 0;
}
...
#pragma omp threadprivate(a)
  Enum ee;
// CHECK: Enum ee;
#pragma omp task untied mergeable depend(out                                                                                      \
                                         : argv[:a][1], (arr)[0:], ([argc][10])argv, b) if (task                                  \
                                                                                            : argc > 0) priority(f) depend(depobj \
                                                                                                                           : y)
...
#pragma omp taskgroup task_reduction(min \
                                     : arr1)
#pragma omp task default(none), private(argc, b) firstprivate(argv, evt) if (argc > 0) final(a > 0) depend(inout                                                      \
                                                                                                           : a, argv[:argc], arr                                      \
                                                                                                           [:a], ([10][argc])argv) priority(23) in_reduction(min      \
                                                                                                                                                             : arr1), \
    detach(evt)
...
#pragma omp task in_reduction(min                                                   \
                              : arr1) depend(iterator(i = 0                         \
                                                      : argc, unsigned j = argc : 0 \
                                                      : a),                         \
                                             out                                    \
                                             : argv[i][j])
  // CHECK-NEXT: #pragma omp taskgroup task_reduction(min: arr1)
