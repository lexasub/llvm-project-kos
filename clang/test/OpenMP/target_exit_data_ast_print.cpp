...
#pragma omp target exit data map(always, release \
                                 : e)
...
#pragma omp target exit data nowait map(release \
                                        : x [0:10], c)
...
#pragma omp target exit data nowait map(always, release \
                                        : e)

#pragma omp target exit data depend(in                                           \
                                    : argc, argv [i:argc], x[:]) nowait map(from \
                                                                            : i)

#pragma omp target exit data nowait depend(in                                                         \
                                           : argc, argv [i:argc], x[:]) map(from                      \
                                                                            : i) if (target exit data \
                                                                                     : j > 0)
...
#pragma omp target exit data nowait map(from                                                 \
                                        : c) depend(in                                       \
                                                    : argc, argv [i:argc], x[:]) map(release \
                                                                                     : d)

#pragma omp target exit data depend(in                                                      \
                                    : argc, argv [i:argc], x[:]) nowait map(always, release \
                                                                            : e)
...
#pragma omp target exit data nowait depend(in                                               \
                                           : argc, argv [i:argc], x[:]) map(always, release \
                                                                            : e)
