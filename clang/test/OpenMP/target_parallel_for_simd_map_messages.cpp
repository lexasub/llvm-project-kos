...
#pragma omp target parallel for simd map(to:) // expected-error {{expected expression}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd map(from \
...
#pragma omp target parallel for simd map(tofrom \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd map(T \
...
#pragma omp target parallel for simd map(always, tofrom \
                                         : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd map(always \
...
#pragma omp target parallel for simd map(tofrom, always \
                                         : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for simd map(always, tofrom \
