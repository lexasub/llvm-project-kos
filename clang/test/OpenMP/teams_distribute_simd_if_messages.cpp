...
#pragma omp teams distribute simd if (simd \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp teams distribute simd if (target \
                                      : argc) // expected-error {{directive name modifier 'target' is not allowed for '#pragma omp teams distribute simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp teams distribute simd if (simd             \
                                      : argc) if (simd \
                                                  : argc) // expected-error {{directive '#pragma omp teams distribute simd' cannot contain more than one 'if' clause with 'simd' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp teams distribute simd if (simd \
...
#pragma omp teams distribute simd if (simd \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp teams distribute simd if (simd \
                                      : argc + z) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp teams distribute simd'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp teams distribute simd if (simd             \
                                      : argc) if (simd \
