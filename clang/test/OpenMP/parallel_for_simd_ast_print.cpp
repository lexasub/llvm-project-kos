...
template <class T, class N> T reduct(T *arr, N num) {
  ...
  T sum = (T)0;
// CHECK: T sum = (T)0;
#pragma omp parallel for simd private(myind, g_ind), linear(ind), aligned(arr) if (parallel \
                                                                                   : num)
...
template <class T> struct S {
  ...
  T result(T *v) const {
    ...
    // CHECK: T val;
    // CHECK: T lin = 0;
#pragma omp parallel for simd private(val) safelen(7) linear(lin \
                                                             : -5) lastprivate(res) simdlen(5) if (7) allocate(lin)
...
template <int LEN> struct S2 {
  static void func(int n, float *a, float *b, float *c) {
    int k1 = 0, k2 = 0;
#pragma omp parallel for simd allocate(k1) safelen(LEN) linear(k1, k2           \
                                                               : LEN) aligned(a \
                                                                              : LEN) simdlen(LEN)
    for (int i = 0; i < n; i++) {
      c[i] = a[i] + b[i];
...
// CHECK: static int *a;
#ifdef OMP5
#pragma omp parallel for simd if (parallel                                                   \
                                  : b) if (simd                                              \
                                           : b) nontemporal(argc, c) lastprivate(conditional \
...
#pragma omp parallel for simd aligned(a                \
                                      : CLEN) linear(a \
                                                     : CLEN) safelen(CLEN) collapse(1) simdlen(CLEN)
