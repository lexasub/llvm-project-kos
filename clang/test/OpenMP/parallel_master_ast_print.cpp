...
#pragma omp parallel master
  a = 2;
#pragma omp parallel master default(none), private(argc, b) firstprivate(argv) shared(d) if (parallel                                                                                                      \
                                                                                             : argc > 0) num_threads(C) copyin(S <T>::TS, thrp) proc_bind(master) reduction(+                              \
                                                                                                                                                                            : c, arr1[argc]) reduction(max \
                                                                                                                                                                                                       : e, arr[:C] [0:10])
  foo();
#pragma omp parallel master if (C) num_threads(s) proc_bind(close) reduction(^                                            \
                                                                             : e, f, arr [0:C][:argc]) reduction(task, && \
                                                                                                                 : g)
  foo();
  return 0;
}
...
#pragma omp parallel master
  ...
  a = 2;
// CHECK-NEXT: a = 2;
#pragma omp parallel master default(none), private(argc, b) firstprivate(argv) if (parallel                                                                                          \
...
// CHECK-NEXT: foo()
#pragma omp parallel master allocate(e) if (b) num_threads(c) proc_bind(close) reduction(^                    \
                                                                                         : e, f) reduction(&& \
                                                                                                           : g, arr [0:argc][:10])
  foo();
