...
#pragma omp parallel master taskloop simd linear(i, lin \
...
void test_linear_colons() {
  int B = 0;
#pragma omp parallel master taskloop simd linear(B \
                                                 : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'}}
#pragma omp parallel master taskloop simd linear(B::ib:B \
                                                 : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp parallel master taskloop simd linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'?}}
#pragma omp parallel master taskloop simd linear(z:B \
                                                 : ib)
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel master taskloop simd linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel master taskloop simd linear(X::x \
                                                 : ::z)
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp parallel master taskloop simd linear(ind2 \
                                                 : L)
  for (i = 0; i < num; ++i) {
    T cur = arr[(int)ind2];
    ind2 += L;
...
#pragma omp parallel master taskloop simd linear(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel master taskloop simd linear(argc : 5) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp parallel master taskloop simd linear(val(a, b) \
                                                 : B::ib)
  for (int k = 0; k < argc; ++k)
    ++k;
...
#pragma omp parallel master taskloop simd allocate(omp_thread_mem_alloc \
                                                   : v) linear(v        \
                                                               : i) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'parallel master taskloop simd' directive}}
    for (int k = 0; k < argc; ++k) {
      i = k;
      v += i;
    }
...
#pragma omp parallel master taskloop simd linear(uval(j))
  for (int k = 0; k < argc; ++k)
    ++k;
  int v = 0;
#pragma omp parallel master taskloop simd linear(v \
                                                 : j)
  for (int k = 0; k < argc; ++k) {
    ++k;
    v += j;
