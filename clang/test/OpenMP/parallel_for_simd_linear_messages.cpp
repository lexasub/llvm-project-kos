...
#pragma omp parallel for simd linear(i, lin \
...
void test_linear_colons() {
  int B = 0;
#pragma omp parallel for simd linear(B \
                                     : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'}}
#pragma omp parallel for simd linear(B::ib:B \
                                     : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp parallel for simd linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'?}}
#pragma omp parallel for simd linear(z:B \
                                     : ib)
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel for simd linear(B \
...
#pragma omp parallel for simd linear(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel for simd linear(argc : 5) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp parallel
  {
    int v = 0;
    int i;
#pragma omp parallel for simd linear(v \
                                     : i)
    for (int k = 0; k < argc; ++k) {
      i = k;
      v += i;
