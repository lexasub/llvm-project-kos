...
#pragma omp taskgroup task_reduction(+ \
                                     : d) allocate(d)
#pragma omp taskloop simd allocate(d) if (taskloop                                                                                                                    \
...
#pragma omp taskgroup task_reduction(+ \
                                     : d)
#pragma omp taskloop simd if (taskloop                                                                \
...
#pragma omp parallel
#ifdef OMP5
#pragma omp taskloop simd private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if (simd                                                                       \
                                                                                                               : argc) mergeable priority(argc) simdlen(16) grainsize(argc) reduction(max \
                                                                                                                                                                                      : a, e) nontemporal(argc, c, d) order(concurrent)
#else
#pragma omp taskloop simd private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if (argc) mergeable priority(argc) simdlen(16) grainsize(argc) reduction(max \
                                                                                                                                                                                    : a, e)
#endif // OMP5
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j)
