...
#pragma omp parallel master taskloop simd lastprivate(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel
#pragma omp parallel master taskloop simd lastprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
  ...
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel
#pragma omp parallel master taskloop simd lastprivate(conditional                     \
...
#pragma omp parallel master taskloop simd allocate(omp_thread_mem_alloc \
                                                   : i) lastprivate(i) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'parallel master taskloop simd' directive}}
    for (int k = 0; k < argc; ++k) {
      i = k;
      v += i;
    }
