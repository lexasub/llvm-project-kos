...
#pragma omp parallel master if (parallel \
                                : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp parallel master'}}
  {
    foo();
  }
#pragma omp parallel master if (parallel             \
                                : argc) if (parallel \
                                            : argc) // expected-error {{directive '#pragma omp parallel master' cannot contain more than one 'if' clause with 'parallel' name modifier}}
  {
    foo();
  }
#pragma omp parallel master if (parallel \
