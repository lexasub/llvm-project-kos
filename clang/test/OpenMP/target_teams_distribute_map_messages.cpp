...
#pragma omp target teams distribute map(tofrom \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute map(T \
...
#pragma omp target teams distribute map(always, tofrom \
                                        : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute map(always \
...
#pragma omp target teams distribute map(tofrom, always \
                                        : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target teams distribute map(always, tofrom \
