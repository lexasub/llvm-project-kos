...
#pragma omp taskgroup allocate(d) task_reduction(+ \
                                                 : d)
#pragma omp taskloop if (taskloop                                                                                  \
...
// CHECK-NEXT: a = 2;
#pragma omp parallel
#pragma omp taskloop private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) shared(g) if (argc) mergeable priority(argc) grainsize(argc) reduction(max \
                                                                                                                                                                   : a, e)
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j) {
#pragma omp cancel taskgroup
