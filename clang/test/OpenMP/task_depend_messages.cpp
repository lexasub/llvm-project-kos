...
#pragma omp task depend(source \
                        : argc) // omp45-error {{expected 'in', 'out', 'inout' or 'mutexinoutset' in OpenMP clause 'depend'}} omp50-error {{expected depend modifier(iterator) or 'in', 'out', 'inout', 'mutexinoutset' or 'depobj' in OpenMP clause 'depend'}}
...
#pragma omp task depend(inout             \
                        : foobool(argc)), \
    depend(in, argc) // omp50-error {{expected addressable lvalue expression, array element, array section or array shaping expression}} omp45-error {{expected addressable lvalue expression, array element or array section}} expected-warning {{missing ':' after dependency type - ignoring}} expected-error {{expected expression}}
#pragma omp task depend(out \
                        : S1) // expected-error {{'S1' does not refer to a value}}
#pragma omp task depend(in \
                        : argv[1][1] = '2')
#pragma omp task depend(in \
                        : vec[1]) // omp50-error {{expected addressable lvalue expression, array element, array section or array shaping expression}} omp45-error {{expected addressable lvalue expression, array element or array section}}
#pragma omp task depend(in \
                        : argv[0])
...
#pragma omp task depend(in \
                        : main)
#pragma omp task depend(in \
                        : a[0]) // omp50-error {{expected addressable lvalue expression, array element, array section or array shaping expression}} omp45-error {{expected addressable lvalue expression, array element or array section}}
#pragma omp task depend(in \
...
#pragma omp task depend(iterator(vector *argc = 0 \
  ...
  foo();
#pragma omp task depend(iterator(unsigned argc = 0 : 10), in \
                        : argc) // omp45-error {{expected 'in', 'out', 'inout' or 'mutexinoutset' in OpenMP clause 'depend'}}
  argc = 0;
#pragma omp task depend(iterator(i = 0 : 10, i = 0 : 10), in \
                        : argv[i]) // omp45-error {{expected 'in', 'out', 'inout' or 'mutexinoutset' in OpenMP clause 'depend'}} omp45-error {{use of undeclared identifier 'i'}} omp50-error {{redefinition of 'i'}} omp50-note {{previous definition is here}}
