...
#pragma omp target parallel for if (target               \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for if (parallel \
                                    : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp target parallel for'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for if (target             \
                                    : argc) if (target \
                                                : argc) // expected-error {{directive '#pragma omp target parallel for' cannot contain more than one 'if' clause with 'target' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for if (parallel             \
                                    : argc) if (parallel \
                                                : argc) // expected-error {{directive '#pragma omp target parallel for' cannot contain more than one 'if' clause with 'parallel' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target parallel for if (target \
...
#pragma omp target parallel for if (target               \
                                    : argc) if (parallel \
                                                : argc) if (argc) // expected-error {{no more 'if' clause is allowed}} expected-note {{previous clause with directive name modifier specified here}} expected-note {{previous clause with directive name modifier specified here}}
  for (i = 0; i < argc; ++i)
    foo();
