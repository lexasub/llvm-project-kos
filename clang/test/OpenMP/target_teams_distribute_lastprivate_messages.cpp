...
#pragma omp target teams distribute allocate(omp_thread_mem_alloc \
                                             : argc) lastprivate(argc) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'target teams distribute' directive}} omp50-error {{allocator must be specified in the 'uses_allocators' clause}}
  for (int k = 0; k < argc; ++k)
    ++k;
...
#pragma omp target teams distribute lastprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp target teams distribute lastprivate(conditional                     \
