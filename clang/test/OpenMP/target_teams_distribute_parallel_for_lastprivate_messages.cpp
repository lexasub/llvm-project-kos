...
#pragma omp target teams distribute parallel for lastprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp target teams distribute parallel for lastprivate(conditional                     \
...
#pragma omp target teams distribute parallel for lastprivate(h) // expected-error {{threadprivate or thread local variable cannot be lastprivate}}
  ...
  int v = 0;
#pragma omp target teams distribute parallel for allocate(omp_thread_mem_alloc \
                                                          : i) lastprivate(i) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'target teams distribute parallel for' directive}}
  for (int k = 0; k < argc; ++k) {
    i = k;
    v += i;
  }
