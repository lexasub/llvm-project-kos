...
#pragma omp parallel for linear(lin \
...
#pragma omp parallel for linear(B::ib:B \
                                : bfoo())
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp parallel for linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
// expected-error@+1 {{unexpected ':' in nested name specifier; did you mean '::'?}}
#pragma omp parallel for linear(z:B \
                                : ib)
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel for linear(B \
  ...
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel for linear(X::x \
                                : ::z)
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel for linear(B, ::z, X::x)
  for (int i = 0; i < 10; ++i)
    ;
#pragma omp parallel for linear(::z) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp parallel for linear(ind2 \
                                : L)
  for (i = 0; i < num; ++i) {
    T cur = arr[(int)ind2];
    ind2 += L;
...
#pragma omp parallel
  {
    int v = 0;
    int i;
#pragma omp parallel for linear(v \
                                : i)
    for (int k = 0; k < argc; ++k) {
      i = k;
      v += i;
