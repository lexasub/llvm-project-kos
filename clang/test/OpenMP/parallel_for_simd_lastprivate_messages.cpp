...
#pragma omp parallel for simd lastprivate(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel for simd lastprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
  ...
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel for simd lastprivate(conditional                        \
  ...
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel for simd lastprivate(foo \
