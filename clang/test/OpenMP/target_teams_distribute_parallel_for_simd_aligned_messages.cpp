...
#pragma omp target teams distribute parallel for simd aligned(B \
                                                              : bfoo())
  for (int i = 0; i < 10; ++i)
    ;

#pragma omp target teams distribute parallel for simd aligned(B::ib:B \
                                                              : bfoo()) // expected-error {{unexpected ':' in nested name specifier; did you mean '::'}}
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp target teams distribute parallel for simd aligned(z:B \
                                                              : bfoo()) // expected-error {{unexpected ':' in nested name specifier; did you mean '::'?}}
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp target teams distribute parallel for simd aligned(X::x \
                                                              : ::z) // expected-error {{integral constant expression must have integral or unscoped enumeration type, not 'int **'}} expected-error {{argument of aligned clause should be array, pointer, reference to array or reference to pointer, not 'B'}}
  for (int i = 0; i < 10; ++i)
    ;

#pragma omp target teams distribute parallel for simd aligned(B, rp, ::z \
                                                              : X::x) // expected-error {{integral constant expression must have integral or unscoped enumeration type, not 'B'}}
  for (int i = 0; i < 10; ++i)
    ;
...
#pragma omp target teams distribute parallel for simd aligned(B::ib, B \
                                                              : C1 + C2) // expected-warning {{aligned clause will be ignored because the requested alignment is not a power of 2}}
  for (int i = 0; i < 10; ++i)
    ;
}
...
#pragma omp target teams distribute parallel for simd aligned(arr \
                                                              : L) // expected-error {{argument to 'aligned' clause must be a strictly positive integer value}}
  for (i = 0; i < num; ++i) {
    T cur = arr[(int)ind2];
    ind2 += L;
    sum += cur;
...
template <int LEN> int test_warn() {
  int *ind2 = 0;
#pragma omp target teams distribute parallel for simd aligned(ind2 \
                                                              : LEN) // expected-error {{argument to 'aligned' clause must be a strictly positive integer value}}
  for (int i = 0; i < 100; i++) {
    ind2 += LEN;
  }
  return 0;
...
#pragma omp target teams distribute parallel for simd aligned(f \
                                                              : j) // expected-note {{initializer of 'j' is not a constant expression}} expected-error {{integral constant expression}}
