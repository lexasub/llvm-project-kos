...
#pragma omp master taskloop lastprivate(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel
#pragma omp master taskloop lastprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
  ...
  for (int k = 0; k < argc; ++k)
    ++k;
#pragma omp parallel
#pragma omp master taskloop lastprivate(conditional                     \
...
#pragma omp parallel
  {
    int v = 0;
    int i;
#pragma omp master taskloop allocate(omp_thread_mem_alloc \
                                     : i) lastprivate(i) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'master taskloop' directive}}
    for (int k = 0; k < argc; ++k) {
      i = k;
      v += i;
    }
