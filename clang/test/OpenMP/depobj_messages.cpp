...
#pragma omp depobj(0) depend(in \
                             : s) // expected-error {{unexpected OpenMP directive '#pragma omp depobj'}}
void foo() {
#pragma omp depobj(0) depend(in \
                             : s) // expected-error {{'omp_depend_t' type not found; include <omp.h>}} expected-error {{expected lvalue expression}}}
}
...
#pragma omp depobj(x) depend(in \
  ...
  do
#pragma omp depobj(x) depend(in \
                             : s) // expected-error {{'#pragma omp depobj' cannot be an immediate substatement}}
    while (argc)
      ;
  do {
#pragma omp depobj(x) depend(in \
...
#pragma omp depobj(x) depend(source \
                             : x) // expected-error {{expected depend modifier(iterator) or 'in', 'out', 'inout' or 'mutexinoutset' in OpenMP clause 'depend'}}
...
#pragma omp depobj(x) depend(in \
                             : argc) destroy // expected-error {{exactly one of 'depend', 'destroy', or 'update' clauses is expected}}
#pragma omp depobj(x) destroy depend(in \
                                     : argc) // expected-error {{exactly one of 'depend', 'destroy', or 'update' clauses is expected}}
#pragma omp depobj(x) depend(in \
                             : argc) update(mutexinoutset) // expected-error {{exactly one of 'depend', 'destroy', or 'update' clauses is expected}}
#pragma omp depobj(x) update(inout) destroy // expected-error {{exactly one of 'depend', 'destroy', or 'update' clauses is expected}}
#pragma omp depobj(x)(x) depend(in \
                                : x) // expected-warning {{extra tokens at the end of '#pragma omp depobj' are ignored}}
...
#pragma omp depend(out \
                   : x) depobj(x) // expected-error {{expected an OpenMP directive}}
...
#pragma omp depobj depend(in \
                          : x)(x) // expected-error {{expected depobj expression}} expected-warning {{extra tokens at the end of '#pragma omp depobj' are ignored}} expected-error {{expected addressable lvalue expression, array element, array section or array shaping expression of non 'omp_depend_t' type}}
