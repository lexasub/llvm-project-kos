...
#pragma omp parallel for schedule(dynamic) default(none) copyin(g) linear(a) allocate(a) lastprivate(conditional \
                                                                                                     : d, e, f) order(concurrent)
...
// CHECK-NEXT: a = 2;
#pragma omp parallel for allocate(argc) private(argc, b), firstprivate(c, d), lastprivate(d, f) collapse(N) schedule(static, N) ordered(N) if (parallel                                                     \
...
int main(int argc, char **argv) {
  ...
  static float g;
#pragma omp threadprivate(g)
#pragma omp parallel for schedule(guided, argc) default(none) copyin(g) linear(a) shared(argc) reduction(task, & \
                                                                                                         : d)
...
// CHECK-NEXT: for (int i = 0; i < 2; ++i)
// CHECK-NEXT: a = 2;
#pragma omp parallel for private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) schedule(auto) ordered if (argc) num_threads(a) default(shared) shared(e) reduction(+             \
                                                                                                                                                                                    : h) linear(a \
                                                                                                                                                                                                : -5)
  for (int i = 0; i < 10; ++i)
    for (int j = 0; j < 10; ++j)
