...
#pragma omp target simd map(to:) // expected-error {{expected expression}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target simd map(from \
...
#pragma omp target simd map(always, tofrom \
                            : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target simd map(always \
...
#pragma omp target simd map(tofrom, always \
                            : x)
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target simd map(always, tofrom \
