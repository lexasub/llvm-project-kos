...
#pragma omp target parallel firstprivate(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  foo();
#pragma omp target parallel firstprivate(argc) allocate, allocate(, allocate(omp_default, allocate(omp_default_mem_alloc, allocate(omp_default_mem_alloc:, allocate(omp_default_mem_alloc                  \
...
#pragma omp target parallel firstprivate(argv[1]) // expected-error {{expected variable name}}
  foo();
#pragma omp target parallel firstprivate(ba) allocate(omp_thread_mem_alloc \
                                                      : ba) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'target parallel' directive}}
  foo();
