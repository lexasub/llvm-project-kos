...
#pragma omp target teams distribute simd allocate(a) private(a) private(this->a) linear(k) allocate(k)
  ...
  void foo() {
    int b, argv, d, c, e, f;
#pragma omp target teams distribute simd private(b), firstprivate(argv) shared(d) reduction(+                  \
                                                                                            : c) reduction(max \
  ...
  void bar() {
    ...
    const int slen1 = 8;
    const int slen2 = 8;
#pragma omp target teams distribute simd simdlen(slen1) safelen(slen2) aligned(arr \
                                                                               : alen)
    for (int k = 0; k < a.a; ++k)
      ++a.a;
  }
...
// CHECK-NEXT: for (int i = 0; i < 10; ++i)
// CHECK-NEXT: foo();
#pragma omp target teams distribute simd private(b), firstprivate(argc) shared(d) reduction(+                  \
                                                                                            : c) reduction(max \
                                                                                                           : e) num_teams(f) thread_limit(d)
  for (int k = 0; k < 10; ++k)
    e += d + argc;
...
// CHECK-NEXT: for (int k = 0; k < 10; ++k)
// CHECK-NEXT: e += d + argc;
#pragma omp target teams distribute simd safelen(clen - 1) aligned(arr \
                                                                   : alen)
  for (int k = 0; k < 10; ++k)
    e += d + argc + arr[k];
...
#pragma omp target teams distribute simd safelen(clen - 1) aligned(arr                                                                                           \
                                                                   : N + 6) if (simd                                                                             \
                                                                                : argc) nontemporal(argc, c, d) order(concurrent) allocate(omp_low_lat_mem_alloc \
                                                                                                                                           : e) firstprivate(e) uses_allocators(omp_low_lat_mem_alloc)
#else
#pragma omp target teams distribute simd safelen(clen - 1) aligned(arr \
                                                                   : N + 6)
#endif // OMP5
