...
#pragma omp target enter data if (1 + 3 - 5) device(arg) map(alloc \
                                                             : gc) nowait
  { ++arg; }
...
// CK1: %{{.+}} = add nsw i32 %{{[^,]+}}, 1
// CK1-NOT: __tgt_target_data_end
#pragma omp target enter data map(close, to \
                                  : lb)
  { ++arg; }
...
// CK1: %{{.+}} = add nsw i32 %{{[^,]+}}, 1
// CK1-NOT: __tgt_target_data_end
#pragma omp target enter data map(always close, to \
                                  : lb)
  { ++arg; }
}
...
#pragma omp target enter data map(present, to \
                                  : lb)
  { ++arg; }
...
// CK1A: %{{.+}} = add nsw i32 %{{[^,]+}}, 1
// CK1A-NOT: __tgt_target_data_end
#pragma omp target enter data map(always close present, to \
                                  : lb)
  { ++arg; }
}
#endif
