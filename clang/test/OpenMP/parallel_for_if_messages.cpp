...
#pragma omp parallel for if (parallel \
  ...
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp parallel for if (parallel \
                             : argc) if (for : argc) // expected-error {{directive name modifier 'for' is not allowed for '#pragma omp parallel for'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp parallel for if (parallel             \
                             : argc) if (parallel \
                                         : argc) // expected-error {{directive '#pragma omp parallel for' cannot contain more than one 'if' clause with 'parallel' name modifier}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp parallel for if (parallel \
