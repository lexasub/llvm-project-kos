...
#pragma omp task affinity(in \
                          : argv[0]) // expected-error {{use of undeclared identifier 'in'}}
...
#pragma omp task affinity(iterator(vector *argc = 0 \
  ...
  foo();
#pragma omp task affinity(iterator(i = 0 : 10, i = 0 : 10) \
                          : argv[i]) // expected-error {{redefinition of 'i'}} expected-note {{previous definition is here}}
