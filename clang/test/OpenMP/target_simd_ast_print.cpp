...
// CHECK-NEXT: a = 2;
#pragma omp target simd allocate(f) private(argc, b), firstprivate(c, d), lastprivate(d, f) collapse(N) if (target              \
                                                                                                            : argc) reduction(+ \
                                                                                                                              : h)
  for (int i = 0; i < 2; ++i)
    for (int j = 0; j < 2; ++j)
...
#pragma omp target simd private(argc, b) firstprivate(argv) if (target                                               \
                                                                : argc > 0) reduction(+                              \
...
#pragma omp target simd if (N) reduction(^                                      \
...
#pragma omp target simd if (target                                                 \
                            : argc > 0) if (simd                                   \
                                            : argc) allocate(omp_default_mem_alloc \
                                                             : f) private(f) uses_allocators(omp_default_mem_alloc)
#else
#pragma omp target simd if (target \
                            : argc > 0)
#endif // OMP5
...
#pragma omp target simd map(always, alloc \
                            : i)
  for (T i = 0; i < 2; ++i) {
  }
...
#pragma omp target simd aligned(arr1 \
                                : N - 1)
  for (T i = 0; i < N; ++i) {
  }
...
#pragma omp target simd private(argc, b), firstprivate(argv, c), lastprivate(d, f) collapse(2) if (target                          \
                                                                                                   : argc) reduction(+             \
                                                                                                                     : h) linear(a \
                                                                                                                                 : -5)
  for (int i = 0; i < 10; ++i)
...
#pragma omp target simd private(argc, b) firstprivate(argv) if (argc > 0) reduction(+                              \
                                                                                    : c, arr1[argc]) reduction(max \
                                                                                                               : e, arr[:5] [0:10])
  for (int i = 0; i < 2; ++i) {
  }
...
#pragma omp target simd if (5) reduction(^                                      \
...
#pragma omp target simd if (target                                                                  \
                            : argc > 0) if (simd                                                    \
                                            : argc) nontemporal(argc, c, d) lastprivate(conditional \
                                                                                        : d, f) order(concurrent)
#else
#pragma omp target simd if (target \
                            : argc > 0)
#endif // OMP5
