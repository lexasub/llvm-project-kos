...
template <typename T> requires(sizeof(T) == 1) struct chars_only {};
...
struct G {
  template <typename T> static T temp;
};
...
template <typename T> requires(sizeof(T) >= 1) struct Z<T> {}; // expected-note{{partial specialization matches [with T = int]}}

template <typename T> requires(sizeof(T) <= 4) struct Z<T> {}; // expected-note{{partial specialization matches [with T = int]}}
