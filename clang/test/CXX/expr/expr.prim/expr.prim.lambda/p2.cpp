...
void odr_used(P &p, Boom<int> boom_int, Boom<float> boom_float,
  ...
  const std::type_info &ti1 = typeid([=, &p]() -> P & { boom_int.tickle(); return p; }()); // expected-note{{in instantiation of member function 'Boom<int>::Boom' requested here}}
  ...
  // unevaluated operand.
  const std::type_info &ti2 = typeid([=]() -> int { boom_float.tickle(); return 0; }()); // expected-error{{lambda expression in an unevaluated operand}}
