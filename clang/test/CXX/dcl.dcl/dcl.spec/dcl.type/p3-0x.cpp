...
class K {
  virtual ~K();
  operator struct S {
  }(); // expected-error{{'K::S' cannot be defined in a type specifier}}
};
...
void f() {
  ...
  for (struct S { S(int) {} } s : Undeclared)
  ...
  } // expected-error {{'U' cannot be defined in a type specifier}}

  (void)(struct V { V(int); }) 0; // expected-error {{'V' cannot be defined in a type specifier}}

  (void)dynamic_cast < struct W {
  } * > ((K *)0); // expected-error {{'W' cannot be defined in a type specifier}}
  (void)static_cast < struct X {
  } * > (0); // expected-error {{'X' cannot be defined in a type specifier}}
  (void)reinterpret_cast < struct Y {
  } * > (0); // expected-error {{'Y' cannot be defined in a type specifier}}
  (void)const_cast < struct Z {
  } * > ((const Z *)0); // expected-error {{'Z' cannot be defined in a type specifier}}
}
