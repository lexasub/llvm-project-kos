...
class A {
protected:
  int x;         // expected-note 3 {{declared}} \
...
class A {
protected:
  int x; // expected-note 3 {{can only access this member on an object of type}}
  static int sx;
  static void test(A &);
};
class B : public A {
...
class A {
protected:
  int x; //expected-note {{declared protected}} // expected-note {{can only access this member on an object of type}}
  static int sx;
  static void test(B &);
};
class B : public A {
...
class A {
protected:
  int x;         // expected-note 2{{declared protected here}} expected-note{{member is declared here}}
...
class A {
protected:
  int x;         // expected-note 3{{member is declared here}}
  static int sx; // expected-note 3{{member is declared here}}
  static void test(D &);
};
class B : public A {
...
protected:
  int foo(); // expected-note 4 {{declared}} expected-note 3 {{can only access this member on an object of type}} expected-note 2 {{member is declared here}}
};
