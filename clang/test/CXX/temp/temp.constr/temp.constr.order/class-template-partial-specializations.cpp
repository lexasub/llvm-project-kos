...
template <typename T> requires C1<T> class B {};

template <typename T> requires(C1<T> && sizeof(T) <= 10) class B<T> {};
...
template <typename T> requires C1<T> class C<T> {};
...
template <typename T> requires C1<T> &&C2<T> struct F<T> {
  enum { value = 2 };
};
...
template <typename T, typename U> requires C2<T> struct I {}; // expected-note {{template is declared here}}

template <typename T, typename U> requires C2<U> struct I<T, U> {}; // expected-error {{class template partial specialization is not more specialized than the primary template}}

template <typename T, typename U> requires C2<T> &&C2<U> struct I<T, U> {};
