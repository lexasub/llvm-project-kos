...
template <typename T> requires C1<T> constexpr bool b() { return false; }

template <typename T> requires(C1<T> && sizeof(T) <= 10) constexpr bool b() { return true; }
...
template <typename T> requires C1<T> bool c() { return true; }

template <typename T> requires C1<T> constexpr bool d() { return false; }
...
template <typename T> requires C1<T> &&C2<T> constexpr int e() { return 2; }

template <typename T> requires C1<T> || C2<T> constexpr int e() { return 3; }
...
template <class T, class U> requires BiggerThan<T, U> void f() {}
// expected-note@-1 {{candidate function [with T = long long, U = int]}}

template <class T, class U> requires BiggerThanInt<T> void f() {}
// expected-note@-1 {{candidate function [with T = long long, U = int]}}
...
template <typename T> requires C3<void> int g() {}

template <typename T> requires C4<void> int g() {}
