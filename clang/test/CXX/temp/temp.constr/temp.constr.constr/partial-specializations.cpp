...
template <typename T> requires(sizeof(T *) >= 4 && sizeof(T) >= 4) struct is_same<T *, T *> { static constexpr bool value = true; };
...
template <same_as<bool> T> requires A<T>::type struct B<T *> {};
// expected-note@-1{{previous}}

template <same_as<bool> T> requires A<T>::type struct B<T *> {};
// expected-error@-1{{redefinition}}

template <same_as T> requires A<T>::type struct B<T *> {};

template <same_as<int> T> requires A<T>::type struct B<T *> {};
} // namespace class_templates

namespace variable_templates {
template <typename T, typename U> requires(sizeof(T) >= 4) constexpr bool is_same_v = false;

template <typename T> requires(sizeof(T *) >= 4 && sizeof(T) >= 4) constexpr bool is_same_v<T *, T *> = true;
