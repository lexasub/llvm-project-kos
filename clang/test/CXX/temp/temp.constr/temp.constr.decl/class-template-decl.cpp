...
template <typename T> requires(bool(T())) struct A;
template <typename U> requires(bool(U())) struct A;

template <typename T>
concept C1 = true;

template <C1 T> requires(bool(T())) struct B;
template <C1 U> requires(bool(U())) struct B;
...
struct AA {
  template <typename T> requires(someFunc(T())) struct A;
};

template <typename U> requires(someFunc(U())) struct AA::A {};

struct AAF {
  template <typename T> requires(someFunc(T())) friend struct AA::A;
};
...
struct TA<N>::AF {
  ...
  template <template <unsigned> class TT> requires TT<N + 0>::happy friend struct TA::A;
};
