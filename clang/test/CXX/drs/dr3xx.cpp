...
namespace ::dr311::X {} // namespace ::dr311::X
} // namespace dr311
...
struct S {
  int n : 1;
} s;                          // expected-note 3{{bit-field is declared here}}
...
template <typename T,
          template <typename> class T1,
          template <typename> class T2>
struct Z2 : T2<typename T1<T>::Type>, // expected-error {{protected}}
            T1<T> {};
...
struct S {
  ...
  typedef enum {} B;
  typedef struct {
  } const C;               // expected-note 0-2{{here}}
...
typedef struct {
} * WithoutLinkage1;                   // expected-note 0-1{{here}}
...
struct B {
  B(const B &) = delete;
}; // expected-error 0-1{{extension}} expected-note 0-1{{here}}
B fb();
const B &b = fb();
#if __cplusplus < 201103L
