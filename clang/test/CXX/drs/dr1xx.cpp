...
struct B : A {
  B();
}; // expected-note +{{would lose const qualifier}} expected-note {{requires 0 arguments}}
const B b1;
...
struct S {
  int n;
} s = {{1}}; // expected-warning {{braces around scalar initializer}}
} // namespace dr155
