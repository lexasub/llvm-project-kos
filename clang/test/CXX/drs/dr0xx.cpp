...
class C : public B {
} c; // expected-error 2 {{inherited virtual base class 'dr7::A' has private destructor}} \
...
class VeryDerivedC : public B, virtual public A {
} vdc;
...
struct Q {};
struct Y {
  friend int operator+(Q, Y (*)());
};
struct X {
  template <typename> static Y f();
};
int m = Q() + X().f<int>;      // ok
int n = Q() + (&(X().f<int>)); // ok
} // namespace member
...
struct B : private A, virtual V {
  int b;
} b; // expected-note 6{{private here}}
