...
struct S {
} *sp = new S::S; // expected-error {{qualified reference to 'S' is a constructor name}}
void f() {
...
template <typename T> struct W : WBase<T> {
  typedef int X;
  int n;
};
...
template <typename T> struct C {
  C()
  throw(typename T::type); // expected-error 1-2{{prior to '::'}}
#if __cplusplus > 201402L
  // expected-error@-2 0-1{{C++17}} expected-note@-2 0-1{{noexcept}}
#endif
};
...
union B {
  constexpr B() = default;
  int a;
}; // expected-error {{not constexpr}} expected-note 2{{candidate}}
union C {
  constexpr C() = default;
  int a, b;
}; // expected-error {{not constexpr}} expected-note 2{{candidate}}
