...
struct NonLiteral {
  NonLiteral() {}
}; // expected-note {{not an aggregate and has no constexpr constructors}}
...
// FIXME: We should reject this.
struct Q {
  operator Abs() { __builtin_unreachable(); }
} q;
#if __cplusplus >= 201703L
...
template <typename F> struct Cnvt2 {
  Cnvt2(F);
  typedef int type;
};
...
void f() {
  ...
  for (struct K{} *p = 0; struct L{} *q = 0;) {
  } // expected-error {{'L' cannot be defined in a condition}}
#if __cplusplus >= 201103L
  using M = struct {};
#endif
  struct N {
...
template <struct R {
} * > struct Y; // expected-error {{cannot be defined in a type specifier}}
} // namespace dr686
