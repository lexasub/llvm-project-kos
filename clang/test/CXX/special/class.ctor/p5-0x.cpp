...
class NotDeleted2d {
  int &&a = 0;
};                 // expected-error {{reference member 'a' binds to a temporary object}} expected-note {{default member init}}
...
class Deleted3b {
  const DefaultedDefCtor1 a[42];
};             // expected-note {{because field 'a' of const-qualified type 'const DefaultedDefCtor1 [42]' would not be initialized}}
Deleted3b d3b; // expected-error {{implicitly-deleted default constructor}}
class Deleted3c {
  const DefaultedDefCtor2 a;
};             // expected-note {{because field 'a' of const-qualified type 'const DefaultedDefCtor2' would not be initialized}}
...
NotDeleted3b nd3b;
class NotDeleted3c {
  const DefaultedDefCtor2 a = DefaultedDefCtor2();
};
NotDeleted3c nd3c;
union NotDeleted3d {
  const int a;
...
ASSERT_NONTRIVIAL(NonTrivialDefCtor3,
                  : virtual Trivial, )
...
ASSERT_NONTRIVIAL(NonTrivialDefCtor5,
                  : NonTrivialDefCtor1, )
