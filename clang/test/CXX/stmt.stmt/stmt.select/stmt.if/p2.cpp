...
void h() {
  if constexpr (4.3) {
  } // expected-error{{conversion from 'double' to 'bool' is not allowed in a converted constant expression}}
  constexpr void *p = nullptr;
  if constexpr (p) {
  } // expected-error{{conversion from 'void *const' to 'bool' is not allowed in a converted constant expression}}
}
} // namespace ccce
...
void in_switch(int n) {
  ...
  switch (n) {
    ...
    case 4:
      return; // expected-error {{cannot jump}}
    else
    default:
      break; // expected-error {{cannot jump}}
  }
}
...
a:
  if constexpr (sizeof(n) == 4) // expected-error {{redefinition}} expected-note {{constexpr if}}
  a:
    goto a; // expected-note 2{{previous}}
  else
  a:
    goto a; // expected-error {{redefinition}} expected-error {{cannot jump}}
}

void evil_things() {
  goto evil_label; // expected-error {{cannot jump}}
  if constexpr (true || ({evil_label: false; })) {
