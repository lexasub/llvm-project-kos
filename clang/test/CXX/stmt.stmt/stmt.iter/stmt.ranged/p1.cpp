...
void g() {
  ...
  for (extern int a : A()) {
  } // expected-error {{loop variable 'a' may not be declared 'extern'}}
  for (static int a : A()) {
  } // expected-error {{loop variable 'a' may not be declared 'static'}}
  for (thread_local int a : A()) {
  } // expected-error {{loop variable 'a' may not be declared 'thread_local'}}
  for (register int a : A()) {
  } // expected-error {{loop variable 'a' may not be declared 'register'}} expected-warning 0-1{{register}} expected-error 0-1{{register}}
  for (constexpr int a : X::C()) {
  } // OK per CWG issue #1204.
...
void DR1442() {
  for (auto a : StdBeginEnd()) {
  } // expected-error {{invalid range expression of type 'StdBeginEnd'; no viable 'begin'}}
}
...
void use(NA::A a, NB::B b, NC::C c, ND::D d, NE::E e, NF::F f) {
  ...
  for (auto x : c) {
  } // expected-error {{no viable 'end' function}}
  for (auto x : d) {
  } // expected-error {{no viable 'begin' function}}
  for (auto x : e) {
  } // expected-error {{no viable 'begin' function}}
  for (auto x : f) {
  } // expected-error {{no viable 'end' function}}
