...
#define USEVARTYPE(type, var) \
  type UNIQ(use)() { return var; }
#define USEVAR(var) USEVARTYPE(int, var)
#define USE(func) \
  void UNIQ(use)() { func(); }
#define USE1(func) \
  void UNIQ(use)() { func(nullptr); }
#define USEMEMFUNC(class, func) \
  void (class ::*UNIQ(use)())() { return &class ::func; }
#define USESTATICMEMFUNC(class, func) \
  void (*UNIQ(use)())() { return &class ::func; }
#define USECLASS(class) \
  void UNIQ(USE)() { class x; }
#define USECOPYASSIGN(class)               \
  class &(class ::*UNIQ(use)())(class &) { \
    return &class ::operator=;             \
  }
#define USEMOVEASSIGN(class)                \
  class &(class ::*UNIQ(use)())(class &&) { \
    return &class ::operator=;              \
  }
...
namespace ns {
__declspec(dllimport) int ExternalGlobal;
}
...
namespace ns {
template <typename T> __declspec(dllimport) int ExternalVarTmpl;
}
...
namespace ns {
__declspec(dllimport) void externalFunc();
}
USE(ns::externalFunc)
...
int NonImportedFunc();
struct ClassWithNonImportedMethod {
  int f();
};
...
struct __declspec(dllimport) ClassWithNonDllImportField {
  using X = ClassWithDtor;
  X t[2];
};
...
struct ClassWithImplicitDtor {
  __declspec(dllimport) ClassWithImplicitDtor();
...
namespace ns {
template <typename T> __declspec(dllimport) void externalFuncTmpl();
}
...
struct __declspec(dllimport) B : A {
  ~B();
};
inline A::~A() {}
inline B::~B() {}
...
template <typename T> struct PartiallySpecializedImportedClassTemplate<T *> {
  void f() {}
};
...
template <typename T> struct __declspec(dllexport) ExplicitlyInstantiatedWithDifferentAttr {
  void f() {}
};
...
template <typename T> struct ExplicitInstantiationDeclImportedDefTemplate {
  void f() {}
  ExplicitInstantiationDeclImportedDefTemplate() {}
};
...
template <typename T> struct __declspec(dllimport) ExplicitInstantiationDeclExportedDefImportedTemplate {
  void f() {}
...
template <typename T> struct PR23770BaseTemplate {
  void f() {}
};
...
template <typename T> struct ExplicitlySpecializedTemplate {
  void func() {}
};
template <> struct ExplicitlySpecializedTemplate<int> {
  void func() {}
};
template <typename T> struct ExplicitlyExportSpecializedTemplate {
  void func() {}
};
...
template <typename T> struct ExplicitlyImportSpecializedTemplate {
  void func() {}
};
template <> struct __declspec(dllimport) ExplicitlyImportSpecializedTemplate<int> { void func(); };

template <typename T> struct ExplicitlyInstantiatedTemplate {
  void func() {}
};
...
template <typename T> struct TopClass {
  void func() {}
};
...
template <typename T> struct ExplicitInstantiationDeclTemplateBase2 {
  void func() {}
};
