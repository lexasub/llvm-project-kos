...
template <template <typename> class, template <typename> class = NoneT, template <typename> class = NoneT, template <typename> class = NoneT> struct Templates;
template <template <typename> class T1,
          template <typename> class T2>
struct Templates<T1, T2> {
...
public:
  static bool Register(char *, char *, int *) {
...
template <
    typename _Pair>
struct _Select1st : unary_function<
                        _Pair,
...
public:
  ...
  typedef typename _Ht::key_equal
      key_equal;
};
} // namespace __gnu_cxx
...
template <typename CCC, typename = typename CCC::key_equal, typename = EEE<CCC>> class III {
};
namespace util {
class
    EEE {
