...
struct __attribute((packed)) PackedD {
  double y = 2;
};
struct Test3 : C, PackedD {
  constexpr Test3() {}
};
...
__attribute__((used)) static nsMemoryImpl sGlobalMemory;
...
struct Ctor {
  constexpr Ctor(int n) : k(n) {}
  int k;
};
struct Mutable {
  constexpr Mutable(int n) : k(n) {}
  mutable int k;
};
...
constexpr long a(bool x) {
  return x ? 0 : (long)&&lbl + (0 && ({lbl: 0; }));
}
void test() {
  static long b = (long)&&lbl - a(false);
lbl:
  return;
}
