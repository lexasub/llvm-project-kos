...
[[
  ...
  ,
      bitand,
  bitor ::compl(! .*_ Cx.!U ^ *R),
  ,
  ,
]] int more_garbage_attr;                 // expected-warning {{unknown attribute 'static' ignored}} \
...
class[[]][[]] attr_after_class_name_decl[[]][[]]; // expected-error {{an attribute list cannot appear here}}

class[[]][[]] attr_after_class_name_definition[[]][[]][[]] {}; // expected-error {{an attribute list cannot appear here}}
...
class base {};
class[[]][[]] final_class alignas(float)[[]] final   // expected-error {{an attribute list cannot appear here}}
...
void bar() {
  ...
  []() [[gnu::noreturn]] { return; }
  (); // expected-warning {{attribute 'noreturn' ignored}} FIXME-error {{should not return}}
  []() [[gnu::noreturn]] { throw; }
  (); // expected-warning {{attribute 'noreturn' ignored}}
  new int[42][[]][5][[]]{};
}
