...
private
int cplusplus_is_not_opencl; // expected-error {{expected unqualified-id}}
...
void test(struct Type *P) {
  ...
  Type = 1 ? (
                 (y
                  : b) // expected-error {{unexpected ':' in nested name specifier}}
                 4)
           : 5;
}
...
void CodeCompleteConsumer::(){
    // expected-error {{xpected unqualified-id}}
}
