...
int test7(int i) {
  switch (i) {
  case false ? 1:
    2 : true ? 1 : 2 : // expected-error {{expected 'case' keyword before expression}}
                       case 10 : 14
                       ? 3
...
void pr19022_2() {
  switch (int x)  // expected-error {{variable declaration in condition must have an initializer}}
  case v1:
  case v2:; // expected-error {{use of undeclared identifier 'v1'}} \
  ...
  void pr19022_3(int x) {
    switch (x)
    case 1:
    case v2:; // expected-error {{use of undeclared identifier 'v2'}}
  }
  ...
  int pr19022_4(int x) {
    ...
    case 1 // expected-error{{expected ':' after 'case'}} expected-note{{previous case defined here}}
        case 1:
      return x; // expected-error{{duplicate case value '1'}}
    }
  }
