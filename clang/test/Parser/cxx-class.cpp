...
public:
  ...
  void m1(){}; // ok, one extra ';' is permitted
  void m() {
    int l = 2;
  };
  ...
  virtual int PR45267() = 0; // ok, despite escaped newline
...
// expected-warning@-2 {{default member initializer for non-static data member is a C++11 extension}}
  ...
  extern struct A a;
  struct A {
  } ::PR41192::a; // ok, no missing ';' here  expected-warning {{extra qualification}}
  ...
  struct C {
  } decltype(D())::c; // expected-error {{'decltype' cannot be used to name a declaration}}
#endif
  } // namespace PR41192
