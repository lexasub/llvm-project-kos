...
struct S {
  ...
  int k10 = 0 < T3<a<b, c>>(d) // expected-error {{expected ';' at end of declaration}}
      ,
      a > (e4);
  int k11 = 0 < 1, c<3>::*ptr;
  ...
  template <typename, int = 0> struct T3 {
    T3(int);
    operator int();
  };
...
namespace CWG325 {
template <int A, typename B> struct T {
  static int i;
  operator int();
};
class C {
...
struct X {
  ...
  template <int, Fn, Fn, Fn> struct T1 {
    enum { val };
  };
  template <int, int, int> struct T2 {
    enum { val };
  };
  template <int, Fn> struct T3 {
    enum { val };
  };
