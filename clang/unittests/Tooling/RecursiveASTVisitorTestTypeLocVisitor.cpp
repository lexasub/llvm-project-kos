...
TEST(RecursiveASTVisitor, VisitsCXXBaseSpecifiersOfSelfReferentialType) {
  ...
  EXPECT_TRUE(Visitor.runOver("template<typename T> class X {};\n"
                              "class Y : public X<Y> {};"));
}
...
TEST(RecursiveASTVisitor, VisitsCompoundLiteralType) {
  ...
  EXPECT_TRUE(
      Visitor.runOver("int f() { return (struct S { int a; }){.a = 0}.a; }",
...
TEST(RecursiveASTVisitor, VisitsObjCPropertyType) {
  ...
  EXPECT_TRUE(
      Visitor.runOver("@class NSNumber; \n"
...
TEST(RecursiveASTVisitor, VisitInvalidType) {
  ...
  EXPECT_FALSE(Visitor.runOver("__typeof__(struct F*) var[invalid];\n",
                               TypeLocVisitor::Lang_C));
}
