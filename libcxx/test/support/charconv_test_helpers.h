...
constexpr auto is_non_narrowing(From a) -> decltype(To{a}, true_type()) {
  return {};
}

template <typename To>
constexpr auto is_non_narrowing(...) -> false_type {
  return {};
}

template <typename X, typename T>
constexpr bool _fits_in(T, true_type /* non-narrowing*/, ...) {
  return true;
}

template <typename X, typename T, typename xl = std::numeric_limits<X> >
constexpr bool _fits_in(T v, false_type, true_type /* T signed*/,
                        true_type /* X signed */) {
...
private:
  static long long fromchars(char const* p, char const* ep, int base,
                             true_type) {
    char* last;
  ...
  static unsigned long long fromchars(char const* p, char const* ep, int base,
                                      false_type) {
    char* last;
