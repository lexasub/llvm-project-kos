...
#define FOPEN_CLOEXEC_FLAG ""
    ...
    if (large_file_ftruncate(fileno(file),
                             static_cast<large_file_offset_t>(size)) == -1) {
  ...
  std::string create_file_dir_symlink(fs::path source_path, fs::path to_path,
  ...
  std::string create_symlink(fs::path source_path, fs::path to_path,
                             bool sanitize_source = true) {
    return create_file_dir_symlink(source_path, to_path, sanitize_source,
                                   false);
  }

  std::string create_directory_symlink(fs::path source_path, fs::path to_path,
                                       bool sanitize_source = true) {
...
public:
  ...
  const std::vector<fs::path> TestFileList = {
      ...
      makePath("dir1/file1"), makePath("dir1/file2")};

  const std::vector<fs::path> TestDirList = {
      makePath("dir1"), makePath("dir1/dir2"), makePath("dir1/dir2/dir3")};
  ...
  const std::vector<fs::path> RecDirIterationList = {
      makePath("dir1/dir2"),       makePath("dir1/file1"),
      makePath("dir1/file2"),      makePath("dir1/dir2/afile3"),
      makePath("dir1/dir2/dir3"),  makePath("dir1/dir2/symlink_to_dir3"),
      makePath("dir1/dir2/file4"), makePath("dir1/dir2/dir3/file5")};
...
const MultiStringType PathList[] = {
    ...
    MKSTR("c:/foo\\bar"),
    MKSTR("//"),
    MKSTR("/finally/we/need/one/really/really/really/really/really/really/"
          "really/long/string")};
...
bool checkCollectionsEqual(Iter1 start1, Iter1 const end1, Iter2 start2,
                           Iter2 const end2) {
...
template <class Iter1, class Iter2>
bool checkCollectionsEqualBackwards(Iter1 const start1, Iter1 end1,
