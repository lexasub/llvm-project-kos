...
TEST_CONSTEXPR_CXX14 bool testComparisons6(const T& t1, const U& t2,
...
void AssertComparisons6ConvertibleToBool() {
  static_assert((std::is_convertible<decltype(std::declval<const T&>() ==
                                              std::declval<const U&>()),
                                     bool>::value),
                "");
  static_assert((std::is_convertible<decltype(std::declval<const T&>() !=
...
template <class T, class U = T>
TEST_CONSTEXPR_CXX14 bool testComparisons2(const T& t1, const U& t2,
                                           bool isEqual) {
  if (isEqual) {
    if (!(t1 == t2))
      return false;
