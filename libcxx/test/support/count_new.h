...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4640) // '%s' construction of local static object is not thread safe (/Zc:threadSafeInit-)
#endif // _MSC_VER
...
struct RequireAllocationGuard {
  ...
  ~RequireAllocationGuard() {
    assert(globalMemCounter.checkOutstandingNewEq(
        static_cast<int>(m_outstanding_new_on_init)));
    std::size_t Expect = m_new_count_on_init + m_req_alloc;
    assert(globalMemCounter.checkNewCalledEq(static_cast<int>(Expect)) ||
           (!m_exactly && globalMemCounter.checkNewCalledGreaterThan(
