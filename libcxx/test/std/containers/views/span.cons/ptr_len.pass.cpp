...
void checkCV() {
  ...
  //  Types the same (dynamic sized)
  {
    ...
    std::span<const int> s2{
        carr, 3}; // a span<const          int> pointing at const int.
    std::span<volatile int> s3{
        varr, 3}; // a span<      volatile int> pointing at volatile int.
    std::span<const volatile int> s4{
        cvarr, 3}; // a span<const volatile int> pointing at const volatile int.
  ...
  //  types different (dynamic sized)
  {
    std::span<const int> s1{arr,
                            3}; // a span<const          int> pointing at int.
    std::span<volatile int> s2{
        arr, 3}; // a span<      volatile int> pointing at int.
    std::span<volatile int> s3{
        arr, 3}; // a span<      volatile int> pointing at const int.
    std::span<const volatile int> s4{
        arr, 3}; // a span<const volatile int> pointing at int.
    std::span<const volatile int> s5{
        carr, 3}; // a span<const volatile int> pointing at const int.
    std::span<const volatile int> s6{
        varr, 3}; // a span<const volatile int> pointing at volatile int.
  ...
  //  types different (static sized)
  {
    std::span<const int, 3> s1{
        arr, 3}; // a span<const          int> pointing at int.
