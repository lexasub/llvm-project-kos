...
int main(int, char**) {
  ...
  {
    bool do_throw = false;
    typedef std::unordered_multiset<Counter<int>, throw_hasher<Counter<int> > >
        set_type;
    ...
    assert(!do_throw);
    assert(set_equal(
        src, set_type({1, 3, 5}, 0, throw_hasher<Counter<int> >(do_throw))));
    assert(set_equal(
        dst, set_type({2, 4, 5}, 0, throw_hasher<Counter<int> >(do_throw))));
  }
#endif
  ...
  {
    typedef std::unordered_multiset<Counter<int>, std::hash<Counter<int> >,
                                    std::equal_to<Counter<int> > >
        first_set_type;
    typedef std::unordered_multiset<Counter<int>, hasher, equal>
        second_set_type;
