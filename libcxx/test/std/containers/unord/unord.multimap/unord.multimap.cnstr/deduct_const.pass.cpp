...
int main(int, char**) {
...

  {
    ...
    ASSERT_SAME_TYPE(decltype(m),
                     std::unordered_multimap<int, long, std::hash<short>,
                                             std::equal_to<int> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
  }

  {
    ...
    ASSERT_SAME_TYPE(
        decltype(m),
        std::unordered_multimap<int, long, std::hash<short>, std::equal_to<> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
  }
  ...
  {
    std::unordered_multimap m{PC{1, 1L}, PC{2, 2L}, PC{1, 1L}, PC{INT_MAX, 1L},
                              PC{3, 1L}};
    ASSERT_SAME_TYPE(decltype(m), std::unordered_multimap<int, long>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
...

  {
    ...
    ASSERT_SAME_TYPE(
        decltype(m),
        std::unordered_multimap<int, long, std::hash<int>, std::equal_to<int>,
                                test_allocator<PC> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
...

  {
    const PC arr[] = {{1, 1}, {2, 2}, {1, 1}, {INT_MAX, 1}, {3, 1}};
    std::unordered_multimap m(std::begin(arr), std::end(arr), 42,
                              std::hash<short>(), test_allocator<PC>(0, 46));
    ASSERT_SAME_TYPE(
        decltype(m),
        std::unordered_multimap<int, long, std::hash<short>, std::equal_to<int>,
                                test_allocator<PC> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
