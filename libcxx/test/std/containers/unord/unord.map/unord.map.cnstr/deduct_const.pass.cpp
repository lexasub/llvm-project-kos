...
int main(int, char**) {
...

  {
    ...
    ASSERT_SAME_TYPE(
        decltype(m),
        std::unordered_map<int, long, std::hash<short>, std::equal_to<int> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
  }

  {
    ...
    ASSERT_SAME_TYPE(
        decltype(m),
        std::unordered_map<int, long, std::hash<short>, std::equal_to<> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
  }

  {
    const PC arr[] = {{1, 1L}, {2, 2L}, {1, 1L}, {INT_MAX, 1L}, {3, 1L}};
    std::unordered_map m(std::begin(arr), std::end(arr), 42, std::hash<short>(),
                         std::equal_to<>(), test_allocator<PC>(0, 41));
    ASSERT_SAME_TYPE(decltype(m),
                     std::unordered_map<int, long, std::hash<short>,
  ...
  {
    std::unordered_map m{PC{1, 1L}, PC{2, 2L}, PC{1, 1L}, PC{INT_MAX, 1L},
                         PC{3, 1L}};
    ASSERT_SAME_TYPE(decltype(m), std::unordered_map<int, long>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
  ...
  {
    std::unordered_map m(
        {PC{1, 1L}, PC{2, 2L}, PC{1, 1L}, PC{INT_MAX, 1L}, PC{3, 1L}}, 42);
    ASSERT_SAME_TYPE(decltype(m), std::unordered_map<int, long>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
                               std::end(expected_m)));
  }
...

  {
    std::unordered_map m(
        {PC{1, 1L}, PC{2, 2L}, PC{1, 1L}, PC{INT_MAX, 1L}, PC{3, 1L}}, 42,
        std::hash<short>(), std::equal_to<>(), test_allocator<PC>(0, 44));
    ASSERT_SAME_TYPE(decltype(m),
                     std::unordered_map<int, long, std::hash<short>,
                                        std::equal_to<>, test_allocator<PC> >);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m),
...

  {
    std::unordered_map m(
        {PC{1, 1L}, PC{2, 2L}, PC{1, 1L}, PC{INT_MAX, 1L}, PC{3, 1L}}, 42,
        std::hash<short>(), test_allocator<PC>(0, 48));
    ASSERT_SAME_TYPE(
        decltype(m),
        std::unordered_map<int, long, std::hash<short>, std::equal_to<int>,
                           test_allocator<PC> >);
