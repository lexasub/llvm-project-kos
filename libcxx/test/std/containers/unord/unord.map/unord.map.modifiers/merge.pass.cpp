...
int main(int, char**) {
  ...
  {
    bool do_throw = false;
    typedef std::unordered_map<Counter<int>, int, throw_hasher<Counter<int> > >
        map_type;
    map_type src({{1, 0}, {3, 0}, {5, 0}}, 0,
                 throw_hasher<Counter<int> >(do_throw));
    map_type dst({{2, 0}, {4, 0}, {5, 0}}, 0,
                 throw_hasher<Counter<int> >(do_throw));
    ...
    assert(!do_throw);
    assert(map_equal(src, map_type({{1, 0}, {3, 0}, {5, 0}}, 0,
                                   throw_hasher<Counter<int> >(do_throw))));
    assert(map_equal(dst, map_type({{2, 0}, {4, 0}, {5, 0}}, 0,
                                   throw_hasher<Counter<int> >(do_throw))));
  }
#endif
  ...
  {
    typedef std::unordered_map<Counter<int>, int, std::hash<Counter<int> >,
                               std::equal_to<Counter<int> > >
        first_map_type;
    typedef std::unordered_map<Counter<int>, int, hasher, equal>
        second_map_type;
    typedef std::unordered_multimap<Counter<int>, int, hasher, equal>
        third_map_type;
