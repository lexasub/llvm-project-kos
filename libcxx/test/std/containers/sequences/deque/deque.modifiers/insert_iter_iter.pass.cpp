...
void test(int P, const C& c0, const C& c2) {
  {
    ...
    assert(static_cast<std::size_t>(distance(c1.begin(), c1.end())) ==
           c1.size());
    i = c1.begin();
    for (int j = 0; j < P; ++j, ++i)
      assert(*i == j);
...
void test_move() {
  ...
  typedef typename C::const_iterator CI;
  {
    MoveOnly mo(0);
    typedef MoveOnly* I;
    c.insert(c.end(), std::move_iterator<I>(&mo),
             std::move_iterator<I>(&mo + 1));
  }
  int j = 0;
    ...
    assert(*i == MoveOnly(j));
  {
    ...
    c.insert(c.end(), std::move_iterator<I>(I(&mo)),
             std::move_iterator<I>(I(&mo + 1)));
  }
  j = 0;
  for (CI i = c.begin(); i != c.end(); ++i, ++j)
    assert(*i == MoveOnly(j));
