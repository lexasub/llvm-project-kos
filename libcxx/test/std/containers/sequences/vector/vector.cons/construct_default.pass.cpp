...
template <class C>
void test1(const typename C::allocator_type& a) {
#if TEST_STD_VER > 14
  static_assert((noexcept(C{typename C::allocator_type{}})), "");
#elif TEST_STD_VER >= 11
  static_assert(
      (noexcept(C(typename C::allocator_type())) ==
...
int main(int, char**) {
...

  {
    ...
    test1<std::vector<int, explicit_allocator<int> > >(
        explicit_allocator<int>{});
    test1<
        std::vector<NotConstructible, explicit_allocator<NotConstructible> > >(
        explicit_allocator<NotConstructible>{});
  }
  {
