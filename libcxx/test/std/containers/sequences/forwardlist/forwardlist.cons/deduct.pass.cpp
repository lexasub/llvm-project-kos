...
int main(int, char**) {
...

  {
    ...
    assert(std::distance(fwl.begin(), fwl.end()) ==
           4); // no size for forward_list
    auto it = fwl.begin();
    assert(*it++ == INT_MAX);
    assert(*it++ == 1L);
    assert(*it++ == 2L);
...

  {
    ...
    static_assert(
        std::is_same_v<decltype(fwl)::allocator_type, std::allocator<A> >, "");
    assert(std::distance(fwl.begin(), fwl.end()) ==
           1); // no size for forward_list
  }

  {
    std::forward_list fwl(
        1, A{}, test_allocator<A>()); // deque (size_type, T, allocator)
    static_assert(std::is_same_v<decltype(fwl)::value_type, A>, "");
    static_assert(
        std::is_same_v<decltype(fwl)::allocator_type, test_allocator<A> >, "");
    assert(std::distance(fwl.begin(), fwl.end()) ==
           1); // no size for forward_list
  }
...

  {
    std::forward_list fwl(
        {1.0, 2.0, 3.0, 4.0},
        test_allocator<double>()); // deque(initializer-list, allocator)
    static_assert(std::is_same_v<decltype(fwl)::value_type, double>, "");
    static_assert(
        std::is_same_v<decltype(fwl)::allocator_type, test_allocator<double> >,
        "");
    assert(std::distance(fwl.begin(), fwl.end()) ==
           4); // no size for forward_list
    auto it = fwl.begin();
...

  {
    ...
    static_assert(std::is_same_v<decltype(fwl)::allocator_type,
                                 std::allocator<long double> >,
                  "");
    assert(std::distance(fwl.begin(), fwl.end()) ==
           0); // no size for forward_list
  }
