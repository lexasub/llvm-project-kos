...
int main(int, char**) {
...

  {
    ...
    static_assert(
        std::is_same_v<decltype(lst)::allocator_type, std::allocator<A> >, "");
    assert(lst.size() == 1);
  }

  {
    std::list lst(1, A{},
                  test_allocator<A>()); // list (size_type, T, allocator)
    static_assert(std::is_same_v<decltype(lst)::value_type, A>, "");
    static_assert(
        std::is_same_v<decltype(lst)::allocator_type, test_allocator<A> >, "");
    assert(lst.size() == 1);
  }
...

  {
    std::list lst(
        {1.0, 2.0, 3.0, 4.0},
        test_allocator<double>()); // list(initializer-list, allocator)
    static_assert(std::is_same_v<decltype(lst)::value_type, double>, "");
    static_assert(
        std::is_same_v<decltype(lst)::allocator_type, test_allocator<double> >,
        "");
    assert(lst.size() == 4);
    auto it = lst.begin();
    std::advance(it, 3);
...

  {
    ...
    static_assert(std::is_same_v<decltype(lst)::allocator_type,
