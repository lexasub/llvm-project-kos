...
bool balanced_allocs() {
  ...
  for (std::vector<int>::const_iterator it = temp2.begin(); it != temp2.end();
       ++it) {
    std::ptrdiff_t const allocs =
        std::count(ca_allocs.begin(), ca_allocs.end(), *it);
    std::ptrdiff_t const deallocs =
...
int main(int, char**) {
  {
    ...
    std::map<int, double, C, A> mo(ar, ar + sizeof(ar) / sizeof(ar[0]), C(5),
                                   A(2));
    std::map<int, double, C, A> m(ar, ar + sizeof(ar) / sizeof(ar[0]) / 2, C(3),
                                  A(7));
    m = mo;
