...
struct rvalue_addable {
  ...
  constexpr rvalue_addable operator()(rvalue_addable&& r,
                                      rvalue_addable const&) {
...
constexpr rvalue_addable operator+(rvalue_addable&& lhs,
                                   rvalue_addable const&) {
  lhs.correctOperatorUsed = true;
  return std::move(lhs);
}

constexpr void test_use_move() {
  rvalue_addable arr[100];
  auto res1 = std::inner_product(arr, arr + 100, arr, rvalue_addable());
  auto res2 =
      std::inner_product(arr, arr + 100, arr, rvalue_addable(),
                         /*predicate=*/rvalue_addable(), do_nothing_op());
...
#if TEST_STD_VER > 17 && (!defined(__cpp_lib_constexpr_string) ||              \
...
test_string()
{
  ...
  assert(std::accumulate(sa, sa + 3, std::string(), std::plus<std::string>()) ==
         "abc");
}
...
TEST_CONSTEXPR_CXX20 void test(Iter1 first1, Iter1 last1, Iter2 first2, T init,
                               T x) {
  assert(std::inner_product(first1, last1, first2, init, std::multiplies<int>(),
                            std::plus<int>()) == x);
}
