...
struct rvalue_subtractable {
  ...
  constexpr rvalue_subtractable operator()(rvalue_subtractable const&,
                                           rvalue_subtractable&& r) {
...
constexpr void test_use_move() {
  ...
  std::adjacent_difference(arr, arr + size, res2,
                           /*predicate=*/rvalue_subtractable());
...
public:
  ...
  TEST_CONSTEXPR_CXX20 friend X operator-(const X& x, const X& y) {
    return X(x.i_ - y.i_);
  }
