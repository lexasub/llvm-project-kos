...
TEST_CONSTEXPR_CXX20 void basic_tests() {
  {
    ...
    std::transform_inclusive_scan(v.begin(), v.end(), v.begin(), std::plus<>(),
                                  add_one{}, size_t{50});
...

  {
    ...
    std::transform_inclusive_scan(v.begin(), v.end(), std::back_inserter(res),
                                  std::plus<>(), add_one{}, size_t{1});
#else
  ...
  //  Make sure that the calculations are done using the init typedef
  {
    ...
    std::transform_inclusive_scan(v.begin(), v.end(), std::back_inserter(res),
                                  std::multiplies<>(), add_one{}, size_t{1});
#else
