...
TEST_CONSTEXPR_CXX20 void test(Iter1 first1, Iter1 last1, T init, BOp bOp,
                               UOp uOp, T x) {
  static_assert(std::is_same_v<T, decltype(std::transform_reduce(
                                      first1, last1, init, bOp, uOp))>);
...
TEST_CONSTEXPR_CXX20 void test() {
  ...
  test(Iter(ia), Iter(ia + sa), 4, std::multiplies<>(), twice(),
       184320); // 64 * 2880
...
TEST_CONSTEXPR_CXX20 void test_return_type() {
  T* p = nullptr;
  static_assert(
      std::is_same_v<Init, decltype(std::transform_reduce(
                               p, p, Init{}, std::plus<>(), identity()))>);
}
...
TEST_CONSTEXPR_CXX20 void test_move_only_types() {
  ...
  assert(60 ==
         std::transform_reduce(
             std::begin(ia), std::end(ia), MoveOnly{0},
