...
TEST_CONSTEXPR_CXX20 void test(Iter1 first, Iter1 last, BOp bop, UOp uop,
                               T init, Iter2 rFirst, Iter2 rLast) {
...
#if TEST_STD_VER < 20 || (defined(__cpp_lib_constexpr_vector) &&               \
...
TEST_CONSTEXPR_CXX20 void test() {
  ...
  for (unsigned int i = 0; i < sa; ++i) {
    test(Iter(ia), Iter(ia + i), std::plus<>(), add_one{}, 0, pResI0,
         pResI0 + i);
    test(Iter(ia), Iter(ia + i), std::multiplies<>(), add_one{}, 0, mResI0,
         mResI0 + i);
    test(Iter(ia), Iter(ia + i), std::plus<>(), std::negate<>(), 0, pResN0,
         pResN0 + i);
    test(Iter(ia), Iter(ia + i), std::multiplies<>(), std::negate<>(), 0,
         mResN0, mResN0 + i);
    test(Iter(ia), Iter(ia + i), std::plus<>(), add_one{}, 2, pResI2,
         pResI2 + i);
    test(Iter(ia), Iter(ia + i), std::multiplies<>(), add_one{}, 2, mResI2,
         mResI2 + i);
    test(Iter(ia), Iter(ia + i), std::plus<>(), std::negate<>(), 2, pResN2,
         pResN2 + i);
...
TEST_CONSTEXPR_CXX20 void basic_tests() {
  {
    ...
    std::transform_exclusive_scan(v.begin(), v.end(), v.begin(), size_t{50},
                                  std::plus<>(), add_one{});
...

  {
    ...
    std::transform_exclusive_scan(v.begin(), v.end(), v.begin(), size_t{30},
                                  std::plus<>(), add_one{});
    for (size_t i = 0; i < v.size(); ++i)
...

  {
    ...
    std::transform_exclusive_scan(v.begin(), v.end(), v.begin(), size_t{40},
                                  std::plus<>(), add_one{});
    for (size_t i = 0; i < v.size(); ++i)
...

  {
    ...
    std::transform_exclusive_scan(v.begin(), v.end(), std::back_inserter(res),
                                  size_t{40}, std::plus<>(), add_one{});
#else
    std::array<size_t, 0> v, res;
    std::transform_exclusive_scan(v.begin(), v.end(), res.begin(), size_t{40},
                                  std::plus<>(), add_one{});
#endif
    assert(res.empty());
  }
  ...
  //  Make sure that the calculations are done using the init typedef
  {
    ...
    std::transform_exclusive_scan(v.begin(), v.end(), std::back_inserter(res),
                                  size_t{1}, std::multiplies<>(), add_one{});
#else
    std::array<size_t, 10> res;
    std::transform_exclusive_scan(v.begin(), v.end(), res.begin(), size_t{1},
                                  std::multiplies<>(), add_one{});
#endif
