...
bool doTest(UsesAllocatorType TExpect, UsesAllocatorType UExpect, TT&& t,
            UU&& u) {
...
void test_pmr_uses_allocator(TT&& t, UU&& u) {
  {
    ...
    assert((doTest<T, U>(UA_None, UA_None, std::forward<TT>(t),
                         std::forward<UU>(u))));
  }
  {
    using T = UsesAllocatorV1<Alloc, 1>;
    using U = UsesAllocatorV2<Alloc, 1>;
    assert((doTest<T, U>(UA_AllocArg, UA_AllocLast, std::forward<TT>(t),
                         std::forward<UU>(u))));
  }
  {
    using T = UsesAllocatorV2<Alloc, 1>;
    using U = UsesAllocatorV3<Alloc, 1>;
    assert((doTest<T, U>(UA_AllocLast, UA_AllocArg, std::forward<TT>(t),
                         std::forward<UU>(u))));
  }
  {
    using T = UsesAllocatorV3<Alloc, 1>;
    using U = NotUsesAllocator<Alloc, 1>;
    assert((doTest<T, U>(UA_AllocArg, UA_None, std::forward<TT>(t),
                         std::forward<UU>(u))));
  }
}
