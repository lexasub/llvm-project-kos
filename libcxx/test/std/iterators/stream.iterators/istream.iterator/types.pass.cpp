...
int main(int, char**) {
  ...
  static_assert(
      (std::is_convertible<
          I1, std::iterator<std::input_iterator_tag, double, std::ptrdiff_t,
                            const double*, const double&> >::value),
      "");
#else
  static_assert(
      (std::is_same<I1::iterator_category, std::input_iterator_tag>::value),
      "");
  ...
  typedef std::istream_iterator<unsigned, wchar_t>
      I2; // unsigned is trivially destructible
#if TEST_STD_VER <= 14
  static_assert(
      (std::is_convertible<
          I2, std::iterator<std::input_iterator_tag, unsigned, std::ptrdiff_t,
                            const unsigned*, const unsigned&> >::value),
      "");
#else
  static_assert(
      (std::is_same<I2::iterator_category, std::input_iterator_tag>::value),
      "");
  ...
  typedef std::istream_iterator<std::string>
      I3; // string is NOT trivially destructible
