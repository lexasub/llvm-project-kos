...
int main(int, char**) {
  ...
  static_assert(
      (std::is_base_of<std::system_error, std::ios_base::failure>::value), "");
  ...
  }
  {
    ...
    std::string what_message(se.what());
    assert(what_message.find(what_arg) != std::string::npos);
    assert(what_message.find(std::iostream_category().message(
               static_cast<int>(std::io_errc::stream))) != std::string::npos);
  }
