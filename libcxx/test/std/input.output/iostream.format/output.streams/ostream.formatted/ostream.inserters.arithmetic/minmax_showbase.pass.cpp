...
static void test(std::ios_base::fmtflags fmt, const char* expected) {
  ...
  ss << std::showbase
     << (std::is_signed<T>::value ? std::numeric_limits<T>::min()
...
int main(int, char**) {
  ...
  const bool long_is_32 = std::integral_constant<
      bool, sizeof(long) == sizeof(int32_t)>::value; // avoid compiler warnings
  const bool long_is_64 = std::integral_constant<
      bool, sizeof(long) == sizeof(int64_t)>::value; // avoid compiler warnings
  const bool long_long_is_64 =
      std::integral_constant<bool, sizeof(long long) == sizeof(int64_t)>::
          value; // avoid compiler warnings
