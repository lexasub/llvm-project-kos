...
TEST_CASE(test_follow_symlinks) {
  ...
  recursive_directory_iterator it(
      testDir, directory_options::follow_directory_symlink, ec);
  TEST_REQUIRE(!ec);
...
TEST_CASE(access_denied_on_recursion_test_case) {
  using namespace fs;
  scoped_test_env env;
  const path testFiles[] = {env.create_dir("dir1"), env.create_dir("dir1/dir2"),
...
TEST_CASE(test_PR35078_with_symlink) {
  ...
  for (auto TC : TestCases) {
    bool SeenNestedFile = false;
    recursive_directory_iterator it =
        SetupState(TC.SkipPermDenied, TC.FollowSymlinks, SeenNestedFile);
    TEST_REQUIRE(!ec);
    TEST_REQUIRE(it != endIt);
...
TEST_CASE(test_PR35078_with_symlink_file) {
  ...
  auto SetupState = [&](bool AllowEAccess, bool FollowSym,
                        bool& SeenNestedFile) {
