...
TEST_CASE(test_error_reporting) {
  ...
  const path cases[] = {"", static_env.DNE, static_env.BadSymlink};
  for (auto& p : cases) {
...
TEST_CASE(basic_space_test) {
  ...
  const path cases[] = {static_env.File, static_env.Dir, static_env.Dir2,
                        static_env.SymlinkToFile, static_env.SymlinkToDir};
  for (auto& p : cases) {
