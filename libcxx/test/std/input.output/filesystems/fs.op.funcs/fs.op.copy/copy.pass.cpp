...
TEST_CASE(test_error_reporting) {
  auto checkThrow = [](path const& f, path const& t,
    ...
    try {
      ...
      return err.path1() == f && err.path2() == t && err.code() == ec;
    }
#else
    ((void)f);
    ((void)t);
...
TEST_CASE(from_is_directory) {
  ...
  const FileInfo files[] = {{"file1", 0}, {"file2", 42}, {"file3", 300}};
  scoped_test_env env;
  const path dir = env.create_dir("dir");
...
TEST_CASE(test_dir_create_symlink) {
  ...
  {
    std::error_code ec = GetTestEC();
    fs::copy(dir, dest, copy_options::create_symlinks | copy_options::recursive,
             ec);
    TEST_CHECK(ErrorIs(ec, std::errc::is_a_directory));
    TEST_CHECK(!exists(dest));
    TEST_CHECK(!is_symlink(dest));
