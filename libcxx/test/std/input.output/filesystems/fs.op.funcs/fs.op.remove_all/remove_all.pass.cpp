...
TEST_CASE(test_error_reporting) {
  auto checkThrow = [](path const& f, const std::error_code& ec) {
    ...
    try {
      ...
      return err.path1() == f && err.path2() == "" && err.code() == ec;
    }
#else
    ((void)f);
    ((void)ec);
  ...
  const path testCases[] = {file_in_bad_dir};
  ...
  // PR#35780
  const path testCasesNonexistant[] = {"", env.make_env_path("dne")};
...
TEST_CASE(basic_remove_all_test) {
  ...
  const path testCases[] = {env.create_file("file", 42),
                            env.create_dir("empty_dir"), nested_link, link};
  for (auto& p : testCases) {
