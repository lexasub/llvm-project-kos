...
int main(int, char**) {
  ...
  std::tm t;
  {
    ...
    t = std::tm();
    I i =
        f.get(I(in), I(in + sizeof(in) / sizeof(in[0]) - 1), ios, err, &t, 'c');
  ...
  }
  {
    ...
    t = std::tm();
    I i =
        f.get(I(in), I(in + sizeof(in) / sizeof(in[0]) - 1), ios, err, &t, 'X');
    assert(i.base() == in + sizeof(in) / sizeof(in[0]) - 1);
    assert(t.tm_sec == 59);
    assert(t.tm_min == 55);
    assert(t.tm_hour == 23);
