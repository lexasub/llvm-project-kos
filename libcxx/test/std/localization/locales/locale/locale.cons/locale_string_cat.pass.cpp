...
#if TEST_STD_VER > 17
  assert(
      (std::has_facet<std::codecvt<char16_t, char8_t, std::mbstate_t> >(loc)));
  assert(
      (std::has_facet<std::codecvt<char32_t, char8_t, std::mbstate_t> >(loc)));
#endif
...
int main(int, char**) {
  {
    ...
    check(loc);
    std::locale loc2(loc, std::string(LOCALE_en_US_UTF_8),
                     std::locale::monetary);
    check(loc2);
  }
