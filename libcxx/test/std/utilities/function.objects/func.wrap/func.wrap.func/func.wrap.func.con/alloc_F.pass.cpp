...
void test_FreeFunction(AllocType& alloc) {
  assert(globalMemCounter.checkOutstandingNewEq(0));
  {
    ...
    assert(
        globalMemCounter.checkOutstandingNewEq(test_alloc_base::alloc_count));
...
void test_for_alloc(Alloc& alloc) {
  ...
  test_MemFunClass<int (MemFunClass::*)(int) const, int(MemFunClass&, int)>(
      alloc);
  test_MemFunClass<int (MemFunClass::*)(int, int) const,
                   int(MemFunClass&, int, int)>(alloc);
}
...
int main(int, char**) {
...
#if TEST_STD_VER >= 11
  {
    ...
    static_assert(
        std::is_constructible<Fn, std::allocator_arg_t, std::allocator<int>,
                              LValueCallable&>::value,
        "");
    static_assert(
        std::is_constructible<Fn, std::allocator_arg_t, std::allocator<int>,
                              LValueCallable>::value,
