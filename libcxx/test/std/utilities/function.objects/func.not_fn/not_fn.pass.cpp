...
struct MoveOnlyCallable {
  ...
  constexpr MoveOnlyCallable(MoveOnlyCallable&& other) : value(other.value) {
...
template <class Ret = bool>
struct CopyCallable {
  constexpr CopyCallable(CopyCallable const& other) : value(other.value) {}

  constexpr CopyCallable(CopyCallable&& other) : value(other.value) {
    other.value = !other.value;
  }
...
struct ConstCallable {
  constexpr ConstCallable(ConstCallable const& other) : value(other.value) {}

  constexpr ConstCallable(ConstCallable&& other) : value(other.value) {
    other.value = !other.value;
  }
...
struct NoExceptCallable {
  ...
  template <class... Args>
  constexpr Ret operator()(Args&&...) noexcept {
    return Ret{value};
  }
...
void throws_in_constructor_test() {
  ...
  struct ThrowsOnCopy {
    ThrowsOnCopy(ThrowsOnCopy const&) { throw 42; }
...
bool call_operator_forwarding_test() {
  ...
  { // test multi arg
    ...
    assert((st.check_call<int&&, const double&&, String&, String&&>(
...
bool test_lwg2767() {
  ...
  struct F {
    constexpr bool operator()(Abstract&&) { return false; }
  };
  {
    Derived d;
    Abstract& a = d;
