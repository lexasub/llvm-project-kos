...
void test(const FromDuration& f, const ToDuration& d) {
  {
...
int main(int, char**) {
...

  {
    ...
    constexpr std::chrono::hours h1 = std::chrono::round<std::chrono::hours>(
        std::chrono::milliseconds(9000000));
    static_assert(h1.count() == 2, "");
    constexpr std::chrono::hours h2 = std::chrono::round<std::chrono::hours>(
        std::chrono::milliseconds(-9000000));
