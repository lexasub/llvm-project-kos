...
int main(int, char**) {
  ...
  static_assert(testComparisons6(year_month{year{1234}, std::chrono::January},
  ...
  for (unsigned i = 1; i < 12; ++i)
    for (unsigned j = 1; j < 12; ++j)
      assert(
          (testComparisons6(year_month{year{1234}, month{i}},
                            year_month{year{1234}, month{j}}, i == j, i < j)));

  //  same month, different years
  for (int i = 1000; i < 20; ++i)
    for (int j = 1000; j < 20; ++j)
      assert((testComparisons6(year_month{year{i}, std::chrono::January},
