...
int main(int, char**) {
  ...
  { // year_month_day_last + months
    ASSERT_NOEXCEPT(std::declval<year_month_day_last>() +
                    std::declval<months>());
    ...
    ASSERT_SAME_TYPE(
        year_month_day_last,
        decltype(std::declval<year_month_day_last>() + std::declval<months>()));
    ASSERT_SAME_TYPE(
        year_month_day_last,
        decltype(std::declval<months>() + std::declval<year_month_day_last>()));

    static_assert(testConstexprMonths(
                      year_month_day_last{year{1}, month_day_last{January}}),
                  "");
  ...
  { // year_month_day_last + years
    ASSERT_NOEXCEPT(std::declval<year_month_day_last>() +
                    std::declval<years>());
    ASSERT_NOEXCEPT(std::declval<years>() +
                    std::declval<year_month_day_last>());

    ASSERT_SAME_TYPE(
        year_month_day_last,
        decltype(std::declval<year_month_day_last>() + std::declval<years>()));
    ASSERT_SAME_TYPE(
        year_month_day_last,
        decltype(std::declval<years>() + std::declval<year_month_day_last>()));
