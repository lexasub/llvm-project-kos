...
int main(int, char**) {
  ...
  static_assert(testComparisons6(month_day{std::chrono::January, day{1}},
                                 month_day{std::chrono::January, day{1}}, true,
                                 false),
                "");

  static_assert(testComparisons6(month_day{std::chrono::January, day{1}},
                                 month_day{std::chrono::January, day{2}}, false,
                                 true),
                "");
  ...
  for (unsigned i = 1; i < 12; ++i)
    for (unsigned j = 1; j < 12; ++j)
      assert((testComparisons6(month_day{month{i}, day{1}},
                               month_day{month{j}, day{1}}, i == j, i < j)));

  //  same month, different days
  for (unsigned i = 1; i < 31; ++i)
    for (unsigned j = 1; j < 31; ++j)
      assert((testComparisons6(month_day{month{2}, day{i}},
