...
int main(int, char**) {
...

  {
    ...
    static_assert(ymwd.weekday_indexed() ==
                      weekday_indexed{std::chrono::Thursday, 1},
                  "");
...

  {
    ...
    static_assert(ymwd.index() == 1, "");
    static_assert(ymwd.weekday_indexed() ==
                      weekday_indexed{std::chrono::Wednesday, 1},
                  "");
    static_assert(ymwd == year_month_weekday{local_days{ymwd}},
                  ""); // round trip
  }

  {
    ...
    static_assert(ymwd.index() == 1, "");
    static_assert(
        ymwd.weekday_indexed() == weekday_indexed{std::chrono::Tuesday, 1}, "");
    static_assert(ymwd == year_month_weekday{local_days{ymwd}},
                  ""); // round trip
  }

  {
    ...
    assert(ymwd.index() == 5);
    assert(
        (ymwd.weekday_indexed() == weekday_indexed{std::chrono::Wednesday, 5}));
