...
void test_uses_allocator_sfinae_evaluation() {
  using BadDefault = DefaultCtorBlowsUp<>;
  {
    ...
    static_assert(!std::is_constructible<Tuple, std::allocator_arg_t, A1<int>,
                                         MoveOnly>::value,
                  "");
  ...
  }
  {
    ...
    static_assert(
        std::is_constructible<Tuple, std::allocator_arg_t, A1<int>, MoveOnly,
                              MoveOnly, BadDefault, BadDefault>::value,
        "");
  }
}
...
int main(int, char**) {
  ...
  {
    std::tuple<MoveOnly, MoveOnly> t(std::allocator_arg, A1<int>(), MoveOnly(0),
                                     MoveOnly(1));
    assert(std::get<0>(t) == 0);
  ...
  }
  {
    ...
    std::tuple<int, alloc_first, alloc_last> t(std::allocator_arg, A1<int>(5),
                                               1, 2, 3);
  ...
  }
  {
    ...
    alloc_first::allocator_constructed = false;
    alloc_last::allocator_constructed = false;
    std::tuple<int, alloc_first, alloc_last> t(tag, A1<int>(5), 1, 2, 3);
    assert(std::get<0>(t) == 1);
    assert(alloc_first::allocator_constructed);
    assert(std::get<1>(t) == alloc_first(2));
    assert(alloc_last::allocator_constructed);
