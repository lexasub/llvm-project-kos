...
int main(int, char**) {
  ...
  count = 0;
  {
    static_assert(!std::is_convertible<ExplicitDerived<int>,
                                       std::tuple<Explicit> >::value,
                  "");
  ...
  count = 0;
  {
    ...
    ((void)foo);
    static_assert(
        std::is_convertible<ExplicitDerived<int>, std::tuple<Implicit> >::value,
        "correct STLs accept this");
#else
    static_assert(!std::is_convertible<ExplicitDerived<int>,
                                       std::tuple<Implicit> >::value,
