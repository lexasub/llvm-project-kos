...
int main(int, char**) {
  ...
  }
  {
    ...
    std::tuple<int, alloc_first> t(std::allocator_arg, A1<int>(5), 10,
                                   alloc_first(15));
  ...
  }
  {
    ...
    std::tuple<int, alloc_first, alloc_last> t(
        std::allocator_arg, A1<int>(5), 1, alloc_first(2), alloc_last(3));
  ...
  }
  {
    ...
    alloc_last::allocator_constructed = false;
    std::tuple<int, alloc_first, alloc_last> t(
        std::allocator_arg, A2<int>(5), 1, alloc_first(2), alloc_last(3));
    assert(std::get<0>(t) == 1);
    assert(!alloc_first::allocator_constructed);
    assert(std::get<1>(t) == alloc_first(2));
    assert(!alloc_last::allocator_constructed);
