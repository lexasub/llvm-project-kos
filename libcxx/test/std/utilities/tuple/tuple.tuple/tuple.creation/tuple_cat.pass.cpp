...
int main(int, char**) {
  ...
  {
    std::tuple<int, MoveOnly> t =
        std::tuple_cat(std::pair<int, MoveOnly>(2, 1));
...

  {
    ...
    std::tuple<int*, MoveOnly> t2(nullptr, 4);
    std::tuple<MoveOnly, MoveOnly, int*, MoveOnly> t3 =
        std::tuple_cat(std::tuple<>(), std::move(t1), std::move(t2));
    assert(std::get<0>(t3) == 1);
    assert(std::get<1>(t3) == 2);
    assert(std::get<2>(t3) == nullptr);
    assert(std::get<3>(t3) == 4);
  }
  {
    ...
    std::tuple<int*, MoveOnly> t2(nullptr, 4);
    std::tuple<MoveOnly, MoveOnly, int*, MoveOnly> t3 =
        std::tuple_cat(std::move(t1), std::tuple<>(), std::move(t2));
    assert(std::get<0>(t3) == 1);
    assert(std::get<1>(t3) == 2);
    assert(std::get<2>(t3) == nullptr);
    assert(std::get<3>(t3) == 4);
  }
  {
    ...
    std::tuple<int*, MoveOnly> t2(nullptr, 4);
    std::tuple<MoveOnly, MoveOnly, int*, MoveOnly> t3 =
        std::tuple_cat(std::move(t1), std::move(t2), std::tuple<>());
    assert(std::get<0>(t3) == 1);
    assert(std::get<1>(t3) == 2);
    assert(std::get<2>(t3) == nullptr);
    assert(std::get<3>(t3) == 4);
  }
  {
    ...
    std::tuple<MoveOnly, MoveOnly, int*, MoveOnly, int> t3 =
        std::tuple_cat(std::move(t1), std::move(t2), std::tuple<int>(5));
    assert(std::get<0>(t3) == 1);
    assert(std::get<1>(t3) == 2);
    assert(std::get<2>(t3) == nullptr);
    assert(std::get<3>(t3) == 4);
  ...
  }
  {
    // See bug #19616.
    auto t1 =
        std::tuple_cat(std::make_tuple(std::make_tuple(1)), std::make_tuple());
    assert(t1 == std::make_tuple(std::make_tuple(1)));

    auto t2 = std::tuple_cat(std::make_tuple(std::make_tuple(1)),
                             std::make_tuple(std::make_tuple(2)));
    assert(t2 == std::make_tuple(std::make_tuple(1), std::make_tuple(2)));
  }
  {
    int x = 101;
    std::tuple<int, const int, int&, const int&, int&&> t(42, 101, x, x,
                                                          std::move(x));
    const auto& ct = t;
    ...
    ASSERT_SAME_TYPE(
        decltype(r),
        std::tuple<int, const int, int&, const int&, int&&, int, const int,
                   int&, const int&, int&&, int, const int, int&, const int&,
  ...
  }
  {
    ...
    std::tuple<NS::Namespaced, NS::Namespaced> t2 = std::tuple_cat(t1, t1);
