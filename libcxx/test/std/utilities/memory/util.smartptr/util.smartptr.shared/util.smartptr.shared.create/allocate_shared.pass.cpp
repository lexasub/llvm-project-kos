...
struct A {
  ...
  A(const A& a) : int_(a.int_), char_(a.char_) { ++count; }
  ~A() { --count; }
...
void test() {
  ...
  {
    int const i = 42;
    std::shared_ptr<Three> p =
        std::allocate_shared<Three>(Alloc(), i, bad, bad);
...
int main(int, char**) {
  ...
  assert(A::count == 0);
  {
    ...
    char c = 'f';
    std::shared_ptr<A> p =
        std::allocate_shared<A>(bare_allocator<void>(), i, c);
    assert(A::count == 1);
    assert(p->get_int() == 68);
    assert(p->get_char() == 'f');
  }
