...
TEST_CONSTEXPR_CXX20 bool test() {
  ...
  {
    A<int> a;
    assert(std::allocator_traits<A<int> >::allocate(a, 10, nullptr) ==
           &a.storage);
  }
  {
    typedef A<IncompleteHolder*> Alloc;
    Alloc a;
    assert(std::allocator_traits<Alloc>::allocate(a, 10, nullptr) ==
           &a.storage);
  }
#endif
  {
    B<int> b;
    assert(std::allocator_traits<B<int> >::allocate(b, 11, nullptr) ==
           &b.storage);
  }
  {
    typedef B<IncompleteHolder*> Alloc;
    Alloc b;
    assert(std::allocator_traits<Alloc>::allocate(b, 11, nullptr) ==
           &b.storage);
  }
