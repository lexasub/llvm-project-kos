...
struct Foo {
  ...
  constexpr Foo(int a, char b, double c, int* count) : Foo(a, b, c) {
    *count += 1;
  }
...
template <typename T, typename = decltype(std::construct_at(
                          (T*)nullptr, 1, 2) // missing arguments for Foo(...)
                                          )>
constexpr bool test_sfinae(int) {
  return false;
}
template <typename T>
