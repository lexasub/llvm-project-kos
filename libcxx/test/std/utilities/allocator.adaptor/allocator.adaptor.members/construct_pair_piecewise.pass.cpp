...
void test_no_inner_alloc() {
  ...
  AllocController P;
  {
    ...
    Alloc CA(P);
    SA A(CA);
    A.construct(ptr, std::piecewise_construct, std::forward_as_tuple(x),
  ...
  P.reset();
  {
    ...
    A.construct(ptr, std::piecewise_construct,
                std::forward_as_tuple(std::move(x)), std::forward_as_tuple(y));
    assert(checkConstruct<int&&>(ptr->first, UA_AllocArg, CA));
...
void test_with_inner_alloc() {
  ...
  AllocController PInner;
  {
    ...
    assert(
        (POuter
             .checkConstruct<std::piecewise_construct_t const&,
