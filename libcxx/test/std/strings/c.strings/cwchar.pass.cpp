...
int main(int, char**) {
  ...
  ASSERT_SAME_TYPE(unsigned long long,
                   decltype(std::wcstoull(L"", (wchar_t**)0, 0)));
  ...
  ASSERT_SAME_TYPE(std::size_t,
                   decltype(std::mbsrtowcs(ws, (const char**)0, s, &mb)));
  ASSERT_SAME_TYPE(std::size_t,
                   decltype(std::wcsrtombs(ns, (const wchar_t**)0, s, &mb)));
  ...
  ASSERT_SAME_TYPE(const wchar_t*,
  ...
  ASSERT_SAME_TYPE(const wchar_t*,
                   decltype(std::wmemchr((const wchar_t*)0, L' ', s)));
#endif
