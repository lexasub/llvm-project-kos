...
constexpr bool test_constexpr() {
  ...
  return (std::find_first_of(FI(std::begin(ic)), FI(std::end(ic)),
         ...
         (std::find_first_of(BI(std::begin(ic)), BI(std::end(ic)),
                             ...
                             eq) == BI(std::end(ic))) &&
         (std::find_first_of(RI(std::begin(ic)), RI(std::end(ic)),
...
int main(int, char**) {
  ...
  assert(std::find_first_of(
             ...
             input_iterator<const int*>(ia + sa),
             forward_iterator<const int*>(ic), forward_iterator<const int*>(ic),
             std::equal_to<int>()) == input_iterator<const int*>(ia + sa));
  assert(std::find_first_of(
             input_iterator<const int*>(ia), input_iterator<const int*>(ia),
             forward_iterator<const int*>(ic),
             forward_iterator<const int*>(ic + 1),
             std::equal_to<int>()) == input_iterator<const int*>(ia));
