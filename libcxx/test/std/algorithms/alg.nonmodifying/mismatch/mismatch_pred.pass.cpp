...
TEST_CONSTEXPR bool test_constexpr() {
  ...
  auto p2 = std::mismatch(std::begin(ia), std::end(ia), std::begin(ic),
                          std::end(ic), eq);
  ...
  auto p4 = std::mismatch(std::begin(ib), std::end(ib), std::begin(ic),
                          std::end(ic), eq);
  if (p4.first != ib + 2 || p4.second != ic + 2)
    return false;

  auto p5 = std::mismatch(II(std::begin(ib)), II(std::end(ib)),
                          II(std::begin(ic)), eq);
  if (p5.first != II(ib + 2) || p5.second != II(ic + 2))
    return false;
  auto p6 = std::mismatch(BI(std::begin(ib)), BI(std::end(ib)),
                          BI(std::begin(ic)), BI(std::end(ic)), eq);
...
int main(int, char**) {
  ...
  assert(
      std::mismatch(II(ia), II(ia + sa), II(ib), II(ib + sb), std::ref(bcp)) ==
      (std::pair<II, II>(II(ia + 3), II(ib + 3))));
