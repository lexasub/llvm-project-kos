...
TEST_CONSTEXPR bool test_constexpr() {
  ...
  auto it =
      std::transform(std::begin(ia), std::end(ia), std::begin(ib), plusOne);

  return it == (std::begin(ib) + std::size(ia)) &&
         *it == 0 // don't overwrite the last value in the output array
         && std::equal(std::begin(ib), it, std::begin(expected),
                       std::end(expected));
}
#endif
...
void test() {
  ...
  int ib[sa] = {0};
  OutIter r = std::transform(InIter(ia), InIter(ia + sa), OutIter(ib), plusOne);
  assert(base(r) == ib + sa);
  assert(ib[0] == 1);
  assert(ib[1] == 2);
  assert(ib[2] == 3);
