...
TEST_CONSTEXPR_CXX20 bool test() {
...

  {
    ...
    std::pop_heap(Iter(input), Iter(input + 5), std::greater<T>());
    assert(input[4] == 1);
    std::pop_heap(Iter(input), Iter(input + 4), std::greater<T>());
    assert(input[3] == 2);
    std::pop_heap(Iter(input), Iter(input + 3), std::greater<T>());
    assert(input[2] == 3);
    std::pop_heap(Iter(input), Iter(input + 2), std::greater<T>());
    assert(input[1] == 4);
    std::pop_heap(Iter(input), Iter(input + 1), std::greater<T>());
    assert(input[0] == 5);
  }
  return true;
}
