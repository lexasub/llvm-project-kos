...
TEST_CONSTEXPR bool test_constexpr() {
  int ia[] = {1, 0, 0, 0};
  int ib[] = {0, 1, 1, 0};
  return (std::is_heap_until(std::begin(ia), std::end(ia),
                             std::greater<int>()) == ia + 1) &&
         (std::is_heap_until(std::begin(ib), std::end(ib),
...
void test() {
  ...
  assert(std::is_heap_until(RI(i1), RI(i1 + 1), std::greater<int>()) ==
         RI(i1 + 1));
