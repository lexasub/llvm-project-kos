...
TEST_CONSTEXPR_CXX20 bool test() {
  ...
  {
    T input[] = {5, 3, 4, 1, 2};
    std::push_heap(Iter(input), Iter(input + 1), std::greater<T>());
    assert(input[0] == 5);
    std::push_heap(Iter(input), Iter(input + 2), std::greater<T>());
    assert(input[0] == 3);
    std::push_heap(Iter(input), Iter(input + 3), std::greater<T>());
    assert(input[0] == 3);
    std::push_heap(Iter(input), Iter(input + 4), std::greater<T>());
    assert(input[0] == 1);
    std::push_heap(Iter(input), Iter(input + 5), std::greater<T>());
    assert(input[0] == 1);
