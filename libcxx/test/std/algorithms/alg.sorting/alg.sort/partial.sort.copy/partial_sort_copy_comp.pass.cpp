...
TEST_CONSTEXPR_CXX20 bool test() {
  ...
  for (int n = 0; n < 15; ++n) {
    for (int m = 0; m < 15; ++m) {
      OutIter it =
          std::partial_sort_copy(Iter(orig), Iter(orig + n), OutIter(work),
                                 OutIter(work + m), std::greater<T>());
      if (n <= m) {
...

  {
    int input[] = {3, 4, 2, 5, 1};
    T output[] = {0, 0, 0};
    std::partial_sort_copy(Iter(input), Iter(input + 5), OutIter(output),
                           OutIter(output + 3), std::greater<T>());
    assert(output[0] == 5);
    assert(output[1] == 4);
    assert(output[2] == 3);
  }
...
int main(int, char**) {
  ...
  static_assert(test<MoveOnly, random_access_iterator<int*>,
                     random_access_iterator<MoveOnly*> >());
