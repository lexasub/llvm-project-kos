...
TEST_CONSTEXPR_CXX20 void test4() {
  ...
  const T b[] = {22, 32, 43, 42, 52};
  {
    ...
    OutIter end = std::set_union(Iter1(a), Iter1(a + 4), Iter2(b), Iter2(b + 5),
                                 OutIter(result), typename T::Comparator());
    assert(std::lexicographical_compare(result, base(end), expected,
                                        expected + 7, T::less) == 0);
  ...
  }
  {
    ...
    OutIter end = std::set_union(Iter1(b), Iter1(b + 5), Iter2(a), Iter2(a + 4),
                                 OutIter(result), typename T::Comparator());
    assert(std::lexicographical_compare(result, base(end), expected,
                                        expected + 7, T::less) == 0);
    for (const T* it = base(end); it != result + 20; ++it) {
