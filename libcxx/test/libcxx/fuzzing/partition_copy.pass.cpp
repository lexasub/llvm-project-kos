...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data,
                                      std::size_t size) {
  auto is_even = [](auto t) { return t % 2 == 0; };

  std::vector<std::uint8_t> v1, v2;
  auto iter = std::partition_copy(
      data, data + size, std::back_inserter<std::vector<std::uint8_t> >(v1),
      std::back_inserter<std::vector<std::uint8_t> >(v2), is_even);
  ((void)iter);
