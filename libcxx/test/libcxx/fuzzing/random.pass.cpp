...
struct ParamTypeHelper {
  ...
  static_assert(
      std::is_same<ResultT,
                   typename ParamT::distribution_type::result_type>::value,
      "");

  static ParamT Create(const uint8_t* data, std::size_t size, bool& OK) {
    constexpr bool select_vector_result =
        std::is_constructible<ParamT, ResultT*, ResultT*, ResultT*>::value;
    constexpr bool select_vector_double =
        std::is_constructible<ParamT, double*, double*>::value;
    constexpr int selector =
        select_vector_result ? 0 : (select_vector_double ? 1 : 2);
    return DispatchAndCreate(std::integral_constant<int, selector>{}, data,
                             size, OK);
  }

  // Vector result
  static ParamT DispatchAndCreate(std::integral_constant<int, 0>,
  ...
  // Vector double
  static ParamT DispatchAndCreate(std::integral_constant<int, 1>,
                                  const std::uint8_t* data, std::size_t size,
                                  bool& OK) {
  ...
  // Default
  static ParamT DispatchAndCreate(std::integral_constant<int, 2>,
                                  const std::uint8_t* data, std::size_t size,
                                  bool& OK) {
    OK = false;
...
struct ParamTypeHelper<std::poisson_distribution<IntT> > {
  ...
  static ParamT Create(const std::uint8_t* data, std::size_t size, bool& OK) {
    ...
    if (vals.empty() || std::isnan(vals[0]) || std::isnan(std::abs(vals[0])) ||
        vals[0] < 0)
      return ParamT{};
    OK = true;
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data,
  ...
  return helper<std::uniform_int_distribution<std::int16_t> >(data, size) ||
         ...
         helper<std::negative_binomial_distribution<std::int16_t> >(data,
                                                                    size) ||
