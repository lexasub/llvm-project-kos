...
int main(int, char**) {
  ...
  {
    __block int param = 0;
    std::function<void(int)> r1(^(int x) {
      param = x;
    });
    r1(4);
    assert(param == 4);
  ...
  }
  {
    ...
    std::function<void(int, int)> r1(^(int x, int y) {
      param1 = x;
      param2 = y;
    });
    r1(3, 4);
  ...
  {
    std::function<int(int, int)> r1(^(int x, int y) {
      return x + y;
    });
    assert(r1(3, 4) == 7);
  }
  ...
  // target
  {
    int (^block)(int) = Block_copy(^(int x) {
      return x + 1;
    });
