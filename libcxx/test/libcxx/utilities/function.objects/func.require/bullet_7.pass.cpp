...
template <class A0, class A1>
void*& freeFunction03(A0&, A1&, ...) {
  return FunctionPtrID<void*&(A0&, A1&, ...),
                       freeFunction03>::setUncheckedCall();
}

template <class A0, class A1, class A2>
void*& freeFunction03(A0&, A1&, A2&) {
  return FunctionPtrID<void*&(A0&, A1&, A2&),
                       freeFunction03>::setUncheckedCall();
}
...
struct Functor03 {
  ...
  typedef Functor03 C;
#define F(Args, ...)                                                           \
  __VA_ARGS__ R operator() Args {                                              \
    return MethodID<R(C::*) Args>::setUncheckedCall();                         \
  }                                                                            \
  __VA_ARGS__ R operator() Args const {                                        \
    return MethodID<R(C::*) Args const>::setUncheckedCall();                   \
  }                                                                            \
  __VA_ARGS__ R operator() Args volatile {                                     \
    return MethodID<R(C::*) Args volatile>::setUncheckedCall();                \
  }                                                                            \
  __VA_ARGS__ R operator() Args const volatile {                               \
    return MethodID<R(C::*) Args const volatile>::setUncheckedCall();          \
  }
#
  F(())
...
template <class... Args>
void*& freeFunction11(Args&&..., ...) {
  return FunctionPtrID<void*&(Args && ..., ...),
                       freeFunction11>::setUncheckedCall();
}
...
template <class ClassType, class CallSig, int Arity, class ObjCaster,
