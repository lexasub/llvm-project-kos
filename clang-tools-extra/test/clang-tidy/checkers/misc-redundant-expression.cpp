...
int TestSimpleEquivalent(int X, int Y) {
  ...
  if (P1.x < P2.x && P1.x < P2.x)
    return 1;
  // CHECK-MESSAGES: :[[@LINE-1]]:19: warning: both sides of operator are equivalent
  if (P2.a[P1.x + 2] < P2.x && P2.a[(P1.x) + (2)] < (P2.x))
    return 1;
...
bool operator<(const MyStruct &lhs, const MyStruct &rhs) { return lhs.x < rhs.x; }   // not modifing
bool operator>(const MyStruct &lhs, MyStruct &rhs) {
  rhs.x--;
  return lhs.x > rhs.x;
}
bool operator||(MyStruct &lhs, const MyStruct &rhs) {
  lhs.x++;
  return lhs.x || rhs.x;
}
...
#define EAGAIN 3
#define NOT_EAGAIN 3
  if (EAGAIN == 0 | EAGAIN == 0)
    return 0;
  if (NOT_EAGAIN == 0 | NOT_EAGAIN == 0)
    return 0;
...
#define SOME_OTHER_MACRO 9
  ...
  if (X < SOME_MACRO && X > SOME_OTHER_MACRO)
    return 1;
  if (X != SOME_MACRO && X >= SOME_OTHER_MACRO)
    return 1;
  if (X != SOME_MACRO && X != SOME_OTHER_MACRO)
    return 1;
  if (X == SOME_MACRO || X == SOME_MACRO_SAME_VALUE)
    return 1;
  if (X == SOME_MACRO || X <= SOME_MACRO_SAME_VALUE)
    return 1;
  if (X == SOME_MACRO || X > SOME_MACRO_SAME_VALUE)
  ...
  if (X == SOME_MACRO && X != SOME_MACRO_SAME_VALUE)
    return 1;
  if (X == SOME_MACRO_SAME_VALUE && X == SOME_MACRO)
    return 1;
...
int TestWithMinMaxInt(int X) {
  ...
  if (X <= 0xFFFFFFFFU && X > 0)
    return 1;
  if (X <= 0xFFFFFFFFU && X > 0U)
    return 1;
