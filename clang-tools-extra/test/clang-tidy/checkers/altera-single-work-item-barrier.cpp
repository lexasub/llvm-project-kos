...
__attribute__((reqd_work_group_size(1, 1, 1))) void __kernel error_barrier_no_id_work_group_size(__global int *foo, int size) {
...
__attribute__((reqd_work_group_size(2, 1, 1))) void __kernel success_barrier_no_id_work_group_size(__global int *foo, int size) {
  for (int j = 0; j < 256; j++) {
    for (int i = 256; i < size; i += 256) {
      foo[j] += foo[j + i];
    }
