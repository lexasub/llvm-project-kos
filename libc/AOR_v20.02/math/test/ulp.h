...
static inline void T(call_nofenv)(const struct fun *f, struct T(args) a, int r,
                                  RT(float) * y, int *ex) {
  *y = T(call)(f, a);
  *ex = 0;
}
...
static inline int T(call_long_fenv)(const struct fun *f, struct T(args) a,
                                    int r, struct RT(ret) * p, RT(float) ygot,
                                    int exgot) {
  if (r != FE_TONEAREST)
    fesetround(r);
  feclearexcept(FE_ALL_EXCEPT);
...
static inline RT(float) T(sum)(struct T(args) a) { return T(reduce)(a, , +); }
...
static int T(cmp)(const struct fun *f, struct gen *gen,
  ...
  for (;;) {
    ...
    if (!ok) {
      ...
      if (fenv && (diff & ~want.ex_may)) {
        ...
        T(printcall)(f, a);
        printf(" is %a %+g ulp, got except 0x%0x", want.y, want.tail, exgot);
        if (diff & exgot)
  ...
  printf(" round %c errlim %g maxerr %g %s cnt %llu cnt1 %llu %g%% cnt2 %llu "
         "%g%% cntfail %llu %g%%\n",
         conf->rc, conf->errlim, maxerr,
         conf->r == FE_TONEAREST ? "+0.5" : "+1.0", (unsigned long long)cnt,
         (unsigned long long)cnt1, 100.0 * cnt1 / cc, (unsigned long long)cnt2,
         100.0 * cnt2 / cc, (unsigned long long)cntfail, 100.0 * cntfail / cc);
  return !!cntfail;
}
