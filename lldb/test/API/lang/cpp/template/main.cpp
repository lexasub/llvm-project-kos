...
template <int Arg> class TestObj {
public:
...
//----------------------------------------------------------------------
enum class EnumType { Member, Subclass };
...
//----------------------------------------------------------------------
template <> class EnumTemplate<EnumType::Member> {
public:
  EnumTemplate(int m) : m_member(m) {}
...
//----------------------------------------------------------------------
template <>
class EnumTemplate<EnumType::Subclass> : public EnumTemplate<EnumType::Member> {
public:
  EnumTemplate(int m) : EnumTemplate<EnumType::Member>(m) {}
};

template <typename FLOAT> struct T1 { FLOAT f = 1.5; };
template <typename FLOAT> struct T2 {
  FLOAT f = 2.5;
  int i = 42;
};
template <typename FLOAT, template <typename> class... Args> class C {
  std::tuple<Args<FLOAT>...> V;
};
...
int main(int argc, char **argv) {
  ...
  C<float, T1> c1;
  C<double, T1, T2> c2;
  return testpos.getArg() - testneg.getArg() + member.getMember() * 2 -
         subclass.getMember(); // Breakpoint 1
}
