...
class CustomString {
public:
  CustomString(const char *buffer) : m_buffer(nullptr) {
    if (buffer) {
...
public:
  ...
  NameAndAddress(const char *N, const char *A)
      : m_name(new CustomString(N)), m_address(new CustomString(A)) {}
  ~NameAndAddress() {}
...
int main(int argc, const char *argv[]) {
  ...
  p.push_back(NameAndAddress(
      "Foo", "10710 Johnson Avenue")); // Set break point at this line.
  p.push_back(NameAndAddress("Arpia", "6956 Florey Street"));
  p.push_back(NameAndAddress(
      "Apple", "1 Infinite Loop")); // Set break point at this line.
  ...
  for (int j = 0; j < p.size(); j++) {
    ...
    std::cout << "Person " << j << " is named " << guy.GetName().GetBuffer()
              << " and lives at " << guy.GetAddress().GetBuffer()
