...
class TaskPoolImpl {
public:
  TaskPoolImpl(uint32_t num_threads) : m_stop(false) {
  ...
  static void Worker(TaskPoolImpl *pool) {
    while (true) {
      ...
      if (pool->m_tasks.empty())
        pool->m_tasks_cv.wait(
            lock, [pool]() { return !pool->m_tasks.empty() || pool->m_stop; });
      if (pool->m_tasks.empty())
        break;
...
class TaskPool {
public:
  // Add a new task to the thread pool and return a std::future belongs for the
  // newly created task. The caller of this function have to wait on the future
  // for this task to complete.
  template <typename F, typename... Args>
  static std::future<typename std::result_of<F(Args...)>::type>
  AddTask(F &&f, Args &&...args) {
    return GetImplementation().AddTask(std::forward<F>(f),
                                       std::forward<Args>(args)...);
  }

  // Run all of the specified tasks on the thread pool and wait until all of
  // them are finished before returning
  template <typename... T> static void RunTasks(T &&...t) {
    RunTaskImpl<T...>::Run(std::forward<T>(t)...);
  }

private:
  static TaskPoolImpl &GetImplementation() {
...
template <typename H, typename... T> struct TaskPool::RunTaskImpl<H, T...> {
  static void Run(H &&h, T &&...t) {
...
int main() {
  ...
  TaskPool::RunTasks([]() { return 1; }, []() { return "aaaa"; });
}
