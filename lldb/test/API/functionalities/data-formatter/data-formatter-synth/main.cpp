...
struct BagOfInts {
  ...
  int y;
  int z;
  BagOfInts(int X) : x(X), y(X + 1), z(X + 2) {}
};
...
struct BagOfFloats {
  ...
  float y;
  float z;
  BagOfFloats(float X) : x(X + 0.334), y(X + 0.500), z(X + 0.667) {}
};
...
struct BagOfBags {
  ...
  BagOfFloats z;
  BagOfFloats q;
  BagOfBags() : x('E'), y('B'), z(1.1), q(20.11) {}
};
...
struct Plenty {
  ...
  Plenty(int N, bool flagA, bool flagB)
      : some_values(new BagOfInts(N)), array(new int[N]), array_size(N),
