...
struct i_am_cool {
  ...
  float floating;
  char character;
  i_am_cool(int I, float F, char C) : integer(I), floating(F), character(C) {}
...
struct i_am_cooler {
  ...
  i_am_cooler(int I1, int I2, float F1, float F2, char C1, char C2)
      : first_cool(I1, F1, C1), second_cool(I2, F2, C2),
        floating((F1 + F2) / 2) {}
};
...
struct Simple {
  ...
  float y;
  char z;
  Simple(int X, float Y, char Z) : x(X), y(Y), z(Z) {}
};
...
struct SimpleWithPointers {
  ...
  SimpleWithPointers(int X, float Y, char Z)
      : x(new int(X)), y(new float(Y)), z(new char[2]) {
    z[0] = Z;
    z[1] = '\0';
  }
};
...
struct Couple {
  ...
  Simple *s;
  Couple(int X, float Y, char Z) : sp(X, Y, Z), s(new Simple(X, Y, Z)) {}
};
...
int main(int argc, const char *argv[]) {
  ...
  *int_array = -1;
  ...
  SimpleWithPointers sparray[] = {SimpleWithPointers(-1, -2, '3'),
