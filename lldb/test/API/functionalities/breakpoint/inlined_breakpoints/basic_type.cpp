...
public:
  ...
  const T &get_a() { return m_a; }

  void set_a(const T &a) { m_a = a; }

  const T &get_b() { return m_b; }
...
void Puts(char const *msg) { std::puts(msg); }
...
int main(int argc, char const *argv[]) {
  ...
  std::printf("(a_union_zero_t) a_union_zero.a = '" T_PRINTF_FORMAT "'\n",
              a_union_zero.a);
  std::printf("(a_union_zero_t*) a_union_zero_ptr = %p, a_union_zero_ptr->a = "
              "'" T_PRINTF_FORMAT "'\n",
              a_union_zero_ptr, a_union_zero_ptr->a);
  std::printf("(a_union_zero_t&) a_union_zero_ref = %p, a_union_zero_ref.a = "
              "'" T_PRINTF_FORMAT "'\n",
  ...
  std::printf("(a_union_nonzero_t) a_union_nonzero.u.a = '" T_PRINTF_FORMAT
              "'\n",
              a_union_nonzero.u.a);
  std::printf("(a_union_nonzero_t*) a_union_nonzero_ptr = %p, "
              "a_union_nonzero_ptr->u.a = '" T_PRINTF_FORMAT "'\n",
              a_union_nonzero_ptr, a_union_nonzero_ptr->u.a);
  std::printf("(a_union_nonzero_t&) a_union_nonzero_ref = %p, "
...
#endif
  Puts("About to exit, break here to check values..."); // Set break point at
                                                        // this line.
  return 0;
}
