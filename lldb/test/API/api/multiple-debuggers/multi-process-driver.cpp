...
bool wait_for_stop_event(SBProcess process, SBListener listener) {
  bool stopped = false;
  while (!stopped) {
    SBEvent event;
...
bool walk_stack_to_main(SBThread thread) {
  if (thread.IsValid() == 0) {
    return false;
  }
...
void *do_one_debugger(void *in) {
  ...
  if (debugger.IsValid()) {
    ...
    SBTarget target =
        debugger.CreateTargetWithFileAndArch(inferior_process_name, "x86_64");
    ...
    if (target.IsValid()) {
      ...
      if (process.IsValid()) {
        ...
        uint32_t rc = broadcaster.AddListener(
            listener, SBProcess::eBroadcastBitStateChanged);
        if (rc == 0) {
        ...
        if (strcmp(process.GetThreadAtIndex(0)
                       .GetFrameAtIndex(0)
                       .GetFunctionName(),
                   "foo") != 0) {
#if DEBUG == 1
          printf(
              "#%lld: First breakpoint did not stop at foo(), instead stopped "
              "at '%s'\n",
              threadnum,
...
int main(int argc, char **argv) {
  ...
  completed_threads_array =
      (bool *)malloc(sizeof(bool) * NUMBER_OF_SIMULTANEOUS_DEBUG_SESSIONS);
  memset(completed_threads_array, 0,
         sizeof(bool) * NUMBER_OF_SIMULTANEOUS_DEBUG_SESSIONS);
  successful_threads_array =
      (bool *)malloc(sizeof(bool) * NUMBER_OF_SIMULTANEOUS_DEBUG_SESSIONS);
  memset(successful_threads_array, 0,
         sizeof(bool) * NUMBER_OF_SIMULTANEOUS_DEBUG_SESSIONS);
  ...
  while (1) {
    ...
    if (iter++ == max_time_to_wait) {
      printf("reached maximum timeout but only %d threads have completed so "
             "far (%d successfully), out of %d.  Exiting.\n",
             total_completed_threads, successful_threads,
             NUMBER_OF_SIMULTANEOUS_DEBUG_SESSIONS);
      break;
    }
