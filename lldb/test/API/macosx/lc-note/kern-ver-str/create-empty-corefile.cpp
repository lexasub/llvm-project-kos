...
void add_lc_note_kern_ver_str_load_command(
    ...
    int payload_file_offset, std::string ident) {
...
int main(int argc, char **argv) {
  if (argc != 2 && argc != 3) {
    fprintf(stderr, "usage: create-empty-corefile <output-core-name> "
                    "[binary-to-copy-uuid-from]\n");
    fprintf(stderr, "Create a Mach-O corefile with an LC_NOTE 'kern ver str' "
                    "load command/payload\n");
    fprintf(stderr, "If a binary is given as a second argument, the Mach-O "
                    "UUID of that file will\n");
  ...
  add_lc_note_kern_ver_str_load_command(load_commands, payload,
                                        header_and_load_cmd_room, ident);
